...
namespace Daysim.Framework.Core {
	public sealed class TimeWindow : ITimeWindow {
		...
		private void MergeBusyMinutes(int start, int end) {
			if (start < 1 || start > Constants.Time.MINUTES_IN_A_DAY || end < 1 || end > Constants.Time.MINUTES_IN_A_DAY) {
				return;
			}
		...
		private List<IMinuteSpan> GetAvailableMinutes() {
			...
			if (_busyMinutes.Count == 0) {
				availableMinutes.Add(new MinuteSpan(1, Constants.Time.MINUTES_IN_A_DAY));
			}
			...
			if (lastBusyMinute != null && lastBusyMinute.End < Constants.Time.MINUTES_IN_A_DAY) {
				availableMinutes.Add(new MinuteSpan(lastBusyMinute.End + 1, Constants.Time.MINUTES_IN_A_DAY));
			}
		...
		public int TotalAvailableMinutes(int inclusiveStart, int inclusiveEnd) {
			...
			if (inclusiveStart < 1 || inclusiveStart > Constants.Time.MINUTES_IN_A_DAY || inclusiveEnd < 1 ||
				 inclusiveEnd > Constants.Time.MINUTES_IN_A_DAY) {
				return 0;
			}
		...
		public int GetAvailableMinute(IRandomUtility randomUtility, int inclusiveStart, int exclusiveEnd,
												Bias bias = Bias.None) {
			if (inclusiveStart < 1 || inclusiveStart > Constants.Time.MINUTES_IN_A_DAY || exclusiveEnd < 1 ||
				 exclusiveEnd > Constants.Time.MINUTES_IN_A_DAY) {
				return 0;
			}
		...
		public int AvailableWindow(int minute, int direction) {
			switch (direction) {
				case Constants.TimeDirection.BEFORE:
					return AdjacentAvailableMinutesBefore(minute);
				case Constants.TimeDirection.AFTER:
		...
		public ITimeWindow IncorporateAnotherTimeWindow(ITimeWindow aOtherTimeWindow) {
			...
			if (firstBusy > 0) {
				SetBusyMinutes(firstBusy, Constants.Time.MINUTES_IN_A_DAY);
			*/
