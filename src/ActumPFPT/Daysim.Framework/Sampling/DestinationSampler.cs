...
//			if (_choiceProbabilityCalculator.ModelIsInEstimationMode) {
		...
		private TSampleItem GetDestination<TSampleItem>(RandomUniform01 randomUniform01, IParcel originParcel, SegmentZone originSegmentZone, IParcel excludedParcel, SegmentZone excludedSegmentZone) where TSampleItem : ISampleItem, new() {
			...
			if (Global.Configuration.DestinationScale == Constants.DestinationScale.PARCEL) {
...
//				}
				...
				if (random > .001)
				{
					...
					{
						if (Global.Configuration.DestinationScale == Constants.DestinationScale.MICRO_ZONE ||
			...
			if (Global.Configuration.DestinationScale != Constants.DestinationScale.ZONE && !destinationParcelIsValid) {
		...
		private sealed class TourSampleItem : SampleItem {
			...
			protected override void SetProbability(DestinationSampler destinationSampler) {
...
//				var setAvailability = destinationSampler._choiceProbabilityCalculator.ModelIsInEstimationMode && ParcelId == destinationSampler._chosenParcel.Id;
				...
				if (Global.Configuration.DestinationScale == Constants.DestinationScale.ZONE) {
					...
					// set chosen alternative availability if model is in estimation mode and the sample item is the chosen alternative
					if (setAvailability) {
						Alternative.Available = Probability > 0 && _weightFromOrigin > Global.Configuration.MinParcelSize;
					}
				...
				// set chosen alternative availability if model is in estimation mode and the sample item is the chosen alternative
				if (setAvailability) {
					Alternative.Available = Probability > 0 && SizeFromDestination >= Global.Configuration.MinParcelSize;
				}
			}
		}
		...
		private sealed class IntermediateStopSampleItem : SampleItem {
			...
			protected override void SetProbability(DestinationSampler destinationSampler) {
				...
				if (Global.Configuration.DestinationScale == Constants.DestinationScale.ZONE) {
					...
					// set chosen alternative availability if model is in estimation mode and the sample item is the chosen alternative
					if (setAvailability) {
						Alternative.Available = (Probability1 > 0 || Probability2 > 0) && _tourWeightFromOrigin > Global.Configuration.MinParcelSize;
					}
				...
				// set chosen alternative availability if model is in estimation mode and the sample item is the chosen alternative
				if (setAvailability) {
					Alternative.Available = (Probability1 > 0 || Probability2 > 0) && SizeFromDestination >= Global.Configuration.MinParcelSize;
				}
			}
		}
		...
		private abstract class SampleItem : ISampleItem {
			...
			public void SetAlternative<TSample>(DestinationSampler destinationSampler, Dictionary<TSample, int> sample, bool isChosenAlternative, AlternativeType alternativeType = AlternativeType.Default) where TSample : ISampleItem {
...
//							alternativeIndex = sample.Count;
					...
					SetProbability(destinationSampler);
				}
			}

			protected abstract void SetProbability(DestinationSampler destinationSampler);
