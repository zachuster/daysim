...
namespace Daysim {
	public sealed class SamplingWeightsCalculator {
		private readonly List<CondensedParcel> _eligibleParcels = new List<CondensedParcel>();
		...
		private Dictionary<int, Zone> _eligibleZones;
		...
		private SamplingWeightsCalculator(string variableName, int mode, int pathType, double vot, int minute) {
			...
			foreach (var parcel in parcelReader) {
				...
				if (_eligibleZones.TryGetValue(parcel.ZoneId, out zone)) {
					_eligibleParcels.Add(parcel.GetCondensedParcel());
				}
		...
		public static void Calculate(string variableName, int mode, int pathType, double vot, int minute) {
			...
			Parallel.For(0, segmentCount, new ParallelOptions { MaxDegreeOfParallelism = Global.LargeDegreeOfParallelism }, segment => CalculateSegment(samplingWeightsCalculator, segment));
		}
		...
		private SegmentZone[] ComputeSegment(int segment) {
			...
			foreach (var parcel in _eligibleParcels) {
				...
				Zone zone = _eligibleZones[parcel.ZoneId];
				if (segmentZone == null) {
				...
				var factor = Math.Exp(sizeFactors[0]) * parcel.EmploymentEducation +
								 ...
								 Math.Exp(sizeFactors[13]) * parcel.LandUseCode19 +
			...
			foreach (var origin in segmentZones.Where(o => o != null)) {
				...
				foreach (var destination in segmentZones.Where(d => d != null)) {
					...
					if (Global.Configuration.DestinationScale == Constants.DestinationScale.ZONE && origin.Id == destination.Id && skimValue.Variable < Constants.EPSILON) {
						if (_variableName == "distance") {
							...
							skimValue.Variable =
								 (_mode == Constants.Mode.WALK) ? 5 :
								 (_mode == Constants.Mode.BIKE) ? 2 :
								 (_mode > Constants.Mode.BIKE && _mode < Constants.Mode.TRANSIT) ? 1 : 0;
						}
					}
