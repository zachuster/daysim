...
namespace Daysim.ChoiceModels {
	public class IntermediateStopLocationModel : ChoiceModel {
		...
		private void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, ITripWrapper trip, int sampleSize, ICondensedParcel choice = null) {
			...
			var availableWindow =
				tour.ParentTour == null
					? personDay.TimeWindow.AvailableWindow(destinationDepartureTime, Constants.TimeDirection.BOTH)
					: tour.ParentTour.TimeWindow.AvailableWindow(destinationDepartureTime, Constants.TimeDirection.BOTH);
...
//				}
				...
				var fkid = (person.Gender == Constants.PersonGender.FEMALE && person.PersonType < Constants.PersonType.DRIVING_AGE_STUDENT && (household.HouseholdTotals.ChildrenAge5Through15 + household.HouseholdTotals.ChildrenUnder5 + household.HouseholdTotals.DrivingAgeStudents > 0)).ToFlag();
//#if TRACE
