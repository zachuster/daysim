...
namespace Daysim.ChoiceModels.H {
	public static class HPersonDayPatternTypeModel {
		...
		private static void Initialize() {
			lock (_lock)
			{
				...
				ChoiceModelHelper.Initialize(ref _helpers[ParallelUtility.GetBatchFromThreadId()], CHOICE_MODEL_NAME,
				                             Global.GetInputPath(Global.Configuration.PersonDayPatternTypeModelCoefficients), TOTAL_ALTERNATIVES,
				                             TOTAL_NESTED_ALTERNATIVES, TOTAL_LEVELS, MAX_PARAMETER);
			}
		}

		public static void Run(PersonDayWrapper personDay, HouseholdDayWrapper householdDay) {
			if (personDay == null) {
		...
		private static void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, PersonDayWrapper personDay, HouseholdDayWrapper householdDay, int choice = Constants.DEFAULT_VALUE) {
        ...
        // int mandatoryKids = 0;
         ...
         //int nonMandatoryKids = 0;
			...
			foreach (PersonDayWrapper pDay in personTypeOrderedPersonDays) {
				i++;
				if (i <= 4) {
                    if (pDay.PatternType == Constants.PatternType.MANDATORY)
                    {
                       mandatoryCount++;
                    }
                    else if (pDay.PatternType == Constants.PatternType.NONMANDATORY) { 
                       nonMandatoryCount++;
                    }
                    else { homeCount++;}
				}
