...
namespace Daysim.ChoiceModels {
	public static class HIntermediateStopLocationModel {
		...
		private static void Initialize(int sampleSize) {
			...
			ChoiceModelHelper.Initialize(ref _helpers[ParallelUtility.GetBatchFromThreadId()], CHOICE_MODEL_NAME, Global.GetInputPath(Global.Configuration.IntermediateStopLocationModelCoefficients), sampleSize, TOTAL_NESTED_ALTERNATIVES, TOTAL_LEVELS, MAX_PARAMETER);
		}

		public static void Run(ITripWrapper trip, IHouseholdDayWrapper householdDay, int sampleSize) {
			if (trip == null) {
		...
		private static void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, ITripWrapper trip, IHouseholdDayWrapper householdDay, int sampleSize, ICondensedParcel choice = null) {
		...
		private sealed class IntermediateStopLocationUtilities : ISamplingUtilities {
			...
			public void SetUtilities(ISampleItem sampleItem, int sampleFrequency) {
				...
				var stopOnFullJointHalfTour = ((_trip.Direction == Constants.TourDirection.ORIGIN_TO_DESTINATION && tour.FullHalfTour1Sequence > 0)
					|| (_trip.Direction == Constants.TourDirection.DESTINATION_TO_ORIGIN && tour.FullHalfTour2Sequence > 0)).ToFlag();
				...
				var transitDiscountFraction = tour.Person.TransitFareDiscountFraction;
				var minute = DayPeriod.BigDayPeriods[DayPeriod.MIDDAY].Start;
				var purpose = tour.DestinationPurpose;

				if (_trip.Direction == Constants.TourDirection.ORIGIN_TO_DESTINATION) {
					GetGenTime(_trip.Household.RandomUtility, Constants.Mode.WALK, purpose, costCoef, timeCoef, transitDiscountFraction, 1, tour.OriginParcel, _trip.OriginParcel, _trip.OriginParcel, out wdis0, out wtime0, out gwtime0);
					GetGenTime(_trip.Household.RandomUtility, Constants.Mode.WALK, purpose, costCoef, timeCoef, transitDiscountFraction, 1, tour.OriginParcel, destinationParcel, _trip.OriginParcel, out wdis1, out wtime1, out gwtime1);
					GetGenTime(_trip.Household.RandomUtility, Constants.Mode.WALK, purpose, costCoef, timeCoef, transitDiscountFraction, 2, tour.OriginParcel, destinationParcel, _trip.OriginParcel, out wdis2, out wtime2, out gwtime2);
					adis0 = ImpedanceRoster.GetValue("distance", Constants.Mode.SOV, Constants.PathType.FULL_NETWORK, Constants.ValueOfTime.DEFAULT_VOT, minute, tour.OriginParcel, _trip.OriginParcel).Variable;
					adis1 = ImpedanceRoster.GetValue("distance", Constants.Mode.SOV, Constants.PathType.FULL_NETWORK, Constants.ValueOfTime.DEFAULT_VOT, minute, tour.OriginParcel, destinationParcel).Variable;
					adis2 = ImpedanceRoster.GetValue("distance", Constants.Mode.SOV, Constants.PathType.FULL_NETWORK, Constants.ValueOfTime.DEFAULT_VOT, minute, destinationParcel, _trip.OriginParcel).Variable;
				}
				else {
					GetGenTime(_trip.Household.RandomUtility, Constants.Mode.WALK, purpose, costCoef, timeCoef, transitDiscountFraction, 1, _trip.OriginParcel, tour.OriginParcel, tour.OriginParcel, out wdis0, out wtime0, out gwtime0);
					GetGenTime(_trip.Household.RandomUtility, Constants.Mode.WALK, purpose, costCoef, timeCoef, transitDiscountFraction, 1, _trip.OriginParcel, destinationParcel, tour.OriginParcel, out wdis1, out wtime1, out gwtime1);
					GetGenTime(_trip.Household.RandomUtility, Constants.Mode.WALK, purpose, costCoef, timeCoef, transitDiscountFraction, 2, _trip.OriginParcel, destinationParcel, tour.OriginParcel, out wdis2, out wtime2, out gwtime2);
					adis0 = ImpedanceRoster.GetValue("distance", Constants.Mode.SOV, Constants.PathType.FULL_NETWORK, Constants.ValueOfTime.DEFAULT_VOT, minute, _trip.OriginParcel, tour.OriginParcel).Variable;
					adis1 = ImpedanceRoster.GetValue("distance", Constants.Mode.SOV, Constants.PathType.FULL_NETWORK, Constants.ValueOfTime.DEFAULT_VOT, minute, _trip.OriginParcel, destinationParcel).Variable;
					adis2 = ImpedanceRoster.GetValue("distance", Constants.Mode.SOV, Constants.PathType.FULL_NETWORK, Constants.ValueOfTime.DEFAULT_VOT, minute, destinationParcel, tour.OriginParcel).Variable;
				}
				...
				switch (tour.Mode) {
					...
					case Constants.Mode.BIKE: {
							...
							if (_trip.Direction == Constants.TourDirection.ORIGIN_TO_DESTINATION) {
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.BIKE, purpose, costCoef, timeCoef, transitDiscountFraction, 1, tour.OriginParcel, destinationParcel, _trip.OriginParcel, out bdis1, out btime1, out gbtime1);
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.BIKE, purpose, costCoef, timeCoef, transitDiscountFraction, 2, tour.OriginParcel, destinationParcel, _trip.OriginParcel, out bdis2, out btime2, out gbtime2);
							}
							else {
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.BIKE, purpose, costCoef, timeCoef, transitDiscountFraction, 1, _trip.OriginParcel, destinationParcel, tour.OriginParcel, out bdis1, out btime1, out gbtime1);
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.BIKE, purpose, costCoef, timeCoef, transitDiscountFraction, 2, _trip.OriginParcel, destinationParcel, tour.OriginParcel, out bdis2, out btime2, out gbtime2);
							}
					...
					case Constants.Mode.SCHOOL_BUS:
						if (wdis1 < WTHRESH && wdis2 < WTHRESH && wdis1 > Constants.EPSILON && wdis2 > Constants.EPSILON) {
							...
							if (_trip.Direction == Constants.TourDirection.ORIGIN_TO_DESTINATION) {
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.SOV, purpose, costCoef, timeCoef, transitDiscountFraction, 1, tour.OriginParcel, destinationParcel, _trip.OriginParcel, out d1Dis1, out d1Time1, out gd1Time1);
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.SOV, purpose, costCoef, timeCoef, transitDiscountFraction, 2, tour.OriginParcel, destinationParcel, _trip.OriginParcel, out d1Dis2, out d1Time2, out gd1Time2);
							}
							else {
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.SOV, purpose, costCoef, timeCoef, transitDiscountFraction, 1, _trip.OriginParcel, destinationParcel, tour.OriginParcel, out d1Dis1, out d1Time1, out gd1Time1);
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.SOV, purpose, costCoef, timeCoef, transitDiscountFraction, 2, _trip.OriginParcel, destinationParcel, tour.OriginParcel, out d1Dis2, out d1Time2, out gd1Time2);
							}
					...
					case Constants.Mode.PARK_AND_RIDE: // destination choice only runs for auto legs of P&R tours
						if (wdis1 < WTHRESH && wdis2 < WTHRESH && wdis1 > Constants.EPSILON && wdis2 > Constants.EPSILON) {
							ttim1 = wtime1;
							ttim2 = wtime2;
							gtim1 = gwtime1;
							...
							if (_trip.Direction == Constants.TourDirection.ORIGIN_TO_DESTINATION) {
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.SOV, purpose, costCoef, timeCoef, transitDiscountFraction, 1, tour.OriginParcel, destinationParcel, _trip.OriginParcel, out d1Dis1, out d1Time1, out gd1Time1, _destinationDepartureTime, parkingTime);
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.SOV, purpose, costCoef, timeCoef, transitDiscountFraction, 2, tour.OriginParcel, destinationParcel, _trip.OriginParcel, out d1Dis2, out d1Time2, out gd1Time2, _destinationDepartureTime, parkingTime);
							}
							else {
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.SOV, purpose, costCoef, timeCoef, transitDiscountFraction, 1, _trip.OriginParcel, destinationParcel, tour.OriginParcel, out d1Dis1, out d1Time1, out gd1Time1, _destinationDepartureTime, parkingTime);
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.SOV, purpose, costCoef, timeCoef, transitDiscountFraction, 2, _trip.OriginParcel, destinationParcel, tour.OriginParcel, out d1Dis2, out d1Time2, out gd1Time2, _destinationDepartureTime, parkingTime);
							}
					...
					case Constants.Mode.HOV2:
						if (wdis1 < WTHRESH && wdis2 < WTHRESH && wdis1 > Constants.EPSILON && wdis2 > Constants.EPSILON) {
							...
							if (_trip.Direction == Constants.TourDirection.ORIGIN_TO_DESTINATION) {
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.HOV2, purpose, costCoef, timeCoef, transitDiscountFraction, 1, tour.OriginParcel, destinationParcel, _trip.OriginParcel, out d2Dis1, out d2Time1, out gd2Time1, _destinationDepartureTime, parkingTime);
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.HOV2, purpose, costCoef, timeCoef, transitDiscountFraction, 2, tour.OriginParcel, destinationParcel, _trip.OriginParcel, out d2Dis2, out d2Time2, out gd2Time2, _destinationDepartureTime, parkingTime);
							}
							else {
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.HOV2, purpose, costCoef, timeCoef, transitDiscountFraction, 1, _trip.OriginParcel, destinationParcel, tour.OriginParcel, out d2Dis1, out d2Time1, out gd2Time1, _destinationDepartureTime, parkingTime);
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.HOV2, purpose, costCoef, timeCoef, transitDiscountFraction, 2, _trip.OriginParcel, destinationParcel, tour.OriginParcel, out d2Dis2, out d2Time2, out gd2Time2, _destinationDepartureTime, parkingTime);
							}
					...
					case Constants.Mode.HOV3:
						if (wdis1 < WTHRESH && wdis2 < WTHRESH && wdis1 > Constants.EPSILON && wdis2 > Constants.EPSILON) {
							...
							if (_trip.Direction == Constants.TourDirection.ORIGIN_TO_DESTINATION) {
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.HOV3, purpose, costCoef, timeCoef, transitDiscountFraction, 1, tour.OriginParcel, destinationParcel, _trip.OriginParcel, out d2Dis1, out d2Time1, out gd2Time1, _destinationDepartureTime, parkingTime);
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.HOV3, purpose, costCoef, timeCoef, transitDiscountFraction, 2, tour.OriginParcel, destinationParcel, _trip.OriginParcel, out d2Dis2, out d2Time2, out gd2Time2, _destinationDepartureTime, parkingTime);
							}
							else {
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.HOV3, purpose, costCoef, timeCoef, transitDiscountFraction, 1, _trip.OriginParcel, destinationParcel, tour.OriginParcel, out d2Dis1, out d2Time1, out gd2Time1, _destinationDepartureTime, parkingTime);
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.HOV3, purpose, costCoef, timeCoef, transitDiscountFraction, 2, _trip.OriginParcel, destinationParcel, tour.OriginParcel, out d2Dis2, out d2Time2, out gd2Time2, _destinationDepartureTime, parkingTime);
							}
					...
					case Constants.Mode.TRANSIT: {
							...
							if (_trip.Direction == Constants.TourDirection.ORIGIN_TO_DESTINATION) {
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.TRANSIT, purpose, costCoef, timeCoef, transitDiscountFraction, 1, tour.OriginParcel, destinationParcel, _trip.OriginParcel, out wtdis1, out wttime1, out gwttime1, _destinationDepartureTime);
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.TRANSIT, purpose, costCoef, timeCoef, transitDiscountFraction, 2, tour.OriginParcel, destinationParcel, _trip.OriginParcel, out wtdis2, out wttime2, out gwttime2, _destinationDepartureTime);
							}
							else {
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.TRANSIT, purpose, costCoef, timeCoef, transitDiscountFraction, 1, _trip.OriginParcel, destinationParcel, tour.OriginParcel, out wtdis1, out wttime1, out gwttime1, _destinationDepartureTime);
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.TRANSIT, purpose, costCoef, timeCoef, transitDiscountFraction, 2, _trip.OriginParcel, destinationParcel, tour.OriginParcel, out wtdis2, out wttime2, out gwttime2, _destinationDepartureTime);
							}
							if (wdis1 < WTHRESH && wdis1 > Constants.EPSILON) {
								...
								if (_trip.Direction == Constants.TourDirection.ORIGIN_TO_DESTINATION) {
									GetGenTime(_trip.Household.RandomUtility, Constants.Mode.SOV, purpose, costCoef, timeCoef, transitDiscountFraction, 1, tour.OriginParcel, destinationParcel, _trip.OriginParcel, out d1Dis1, out d1Time1, out gd1Time1, _destinationDepartureTime);
								}
								else {
									GetGenTime(_trip.Household.RandomUtility, Constants.Mode.SOV, purpose, costCoef, timeCoef, transitDiscountFraction, 1, _trip.OriginParcel, destinationParcel, tour.OriginParcel, out d1Dis1, out d1Time1, out gd1Time1, _destinationDepartureTime);
								}
							...
							if (wdis2 < WTHRESH && wdis2 > Constants.EPSILON) {
								...
								if (_trip.Direction == Constants.TourDirection.ORIGIN_TO_DESTINATION) {
									GetGenTime(_trip.Household.RandomUtility, Constants.Mode.SOV, purpose, costCoef, timeCoef, transitDiscountFraction, 2, tour.OriginParcel, destinationParcel, _trip.OriginParcel, out d1Dis2, out d1Time2, out gd1Time2, _destinationDepartureTime);
								}
								else {
									GetGenTime(_trip.Household.RandomUtility, Constants.Mode.SOV, purpose, costCoef, timeCoef, transitDiscountFraction, 2, _trip.OriginParcel, destinationParcel, tour.OriginParcel, out d1Dis2, out d1Time2, out gd1Time2, _destinationDepartureTime);
								}
				...
				else if (tour.Mode == Constants.Mode.TRANSIT && wttime1 > Constants.EPSILON) {
					wta = wta + 1;
				}
				...
				else if (tour.Mode == Constants.Mode.TRANSIT && wttime2 > Constants.EPSILON) {
					wta = wta + 1;
				}
				...
				//				 if (tour.Mode == Constants.Mode.TRANSIT && wtu1 > 0) {
				//				 	int check3 = 1;
				//				 }
				//
				//				int check = 0;
				...
				var adultFemaleWithChildren = (person.Gender == Constants.PersonGender.FEMALE && person.PersonType < Constants.PersonType.DRIVING_AGE_STUDENT && (household.HouseholdTotals.ChildrenAge5Through15 + household.HouseholdTotals.ChildrenUnder5 + household.HouseholdTotals.DrivingAgeStudents > 0)).ToFlag();
				//#if TRACE
				...
				//				var essc = _trip.DestinationPurpose == Constants.Purpose.ESCORT && _trip.Tour.DestinationPurpose == Constants.Purpose.SCHOOL ? 1 : 0;
				//				var shsh = shoppingDestinationPurposeFlag == 1 && _trip.Tour.DestinationPurpose == Constants.Purpose.SHOPPING ? 1 : 0; // shoppingDestinationPurposeFlag trip on shoppingDestinationPurposeFlag tour
				//				var bman = _trip.Direction == 1 && (_trip.Tour.DestinationPurpose == Constants.Purpose.WORK || _trip.Tour.DestinationPurpose == Constants.Purpose.SCHOOL) ? 1 : 0;
				//
				...
				alternative.AddUtilityTerm(6, detourDistanceCubedInHundredsOfMilesCubed);
				...
				switch (_trip.DestinationPurpose) {
					...
					case Constants.Purpose.SOCIAL:
						...
						alternative.AddUtilityTerm(153, socialStopPurposeFlag * destinationParcel.StudentsK12);
					...
					case Constants.Purpose.RECREATION:
						...
						alternative.AddUtilityTerm(136, recreationStopPurposeFlag * destinationParcel.EmploymentMedical);
			...
			private static void GetGenTime(IRandomUtility randomUtility, int mode, int purpose, double costCoef, double timeCoef, double transitDiscountFraction, int leg, ICondensedParcel tripOrigin, ICondensedParcel tripDestination, ICondensedParcel tourOrigin, out double dis, out double tim, out double gtim, int minute = -1, double parkHours = 0.0) {
				...
				if (path.Available) {
					...
					dis = 0D;
					tim = 0D;
					gtim = Constants.GENERALIZED_TIME_UNAVAILABLE;
				}
			}
			...
			public static bool ShouldRunInEstimationModeForModel(ITripWrapper trip, ITourWrapper tour, int tourMode) {
				...
				if (Global.Configuration.IsInEstimationMode) {
					...
					if (tourMode == Constants.Mode.NONE) {
						excludeReason = 1;
					}
					else if (tourMode >= Constants.Mode.OTHER) {
						excludeReason = 2;
					}
