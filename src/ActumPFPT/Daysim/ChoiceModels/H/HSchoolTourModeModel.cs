...
namespace Daysim.ChoiceModels {
	public static class HSchoolTourModeModel {
		...
		private static readonly int[] _nestedAlternativeIds = new[] { 0, 19, 19, 20, 21, 21, 22, 0, 23 };
		private static readonly int[] _nestedAlternativeIndexes = new[] { 0, 0, 0, 1, 2, 2, 3, 0, 4 };
		...
		private static void Initialize() {
			lock (_lock) {
				...
				ChoiceModelHelper.Initialize(ref _helpers[ParallelUtility.GetBatchFromThreadId()], CHOICE_MODEL_NAME, Global.GetInputPath(Global.Configuration.SchoolTourModeModelCoefficients),
													  Constants.Mode.TOTAL_MODES, TOTAL_NESTED_ALTERNATIVES, TOTAL_LEVELS, MAX_PARAMETER);
			}
		}
		...
		private static void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, ITourWrapper tour, List<PathTypeModel> pathTypeModels, ICondensedParcel destinationParcel, int householdCars, int choice = Constants.DEFAULT_VALUE) {
