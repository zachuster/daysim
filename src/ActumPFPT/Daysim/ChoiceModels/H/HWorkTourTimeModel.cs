...
namespace Daysim.ChoiceModels {
	public static class HWorkTourTimeModel {
		private const string CHOICE_MODEL_NAME = "HWorkTourTimeModel";
		...
		private static void Initialize() {
			...
			ChoiceModelHelper.Initialize(ref _helpers[ParallelUtility.GetBatchFromThreadId()], CHOICE_MODEL_NAME, Global.Configuration.WorkTourTimeModelCoefficients, TourTime.TOTAL_TOUR_TIMES, TOTAL_NESTED_ALTERNATIVES, TOTAL_LEVELS, MAX_PARAMETER);
		}

		public static void Run(TourWrapper tour, int knownArrivalTime = 0, int knownDepartureTime = 0) {
			if (tour == null) {
			...
			if (_helpers[ParallelUtility.GetBatchFromThreadId()].ModelIsInEstimationMode) {
				if (tour.DestinationParcel == null || tour.OriginParcel == null || tour.Mode <= Constants.Mode.NONE || tour.Mode >= Constants.Mode.SCHOOL_BUS) {
					return;
				}
		...
		private static void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, TourWrapper tour, int knownArrivalTime = 0, int knownDepartureTime = 0, TourTime choice = null) {
