...
namespace Daysim.ChoiceModels.H {
	public static class HPartialJointHalfTourParticipationModel {
		...
		private static void Initialize() {
			lock (_lock) {
				...
				ChoiceModelHelper.Initialize(ref _helpers[ParallelUtility.GetBatchFromThreadId()], CHOICE_MODEL_NAME,
													  Global.GetInputPath(Global.Configuration.PartialJointHalfTourParticipationModelCoefficients),
													  TOTAL_ALTERNATIVES, TOTAL_NESTED_ALTERNATIVES, TOTAL_LEVELS, MAX_PARAMETER);
			}
		}

		public static int[] Run(HouseholdDayWrapper householdDay, int jHTSimulated, int genChoice, bool[,] jHTAvailable, bool[] pHTAvailable, bool[,] jHTParticipation) {
		...
		private static void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, HouseholdDayWrapper householdDay, int jHTSimulated, int genChoice, bool[,] jHTAvailable, bool[] pHTAvailable, int[][] altParticipants, int choice = Constants.DEFAULT_VALUE) {
			...
			for (var t2 = 1; t2 <= 5; t2++) {
				for (var t1 = 1; t1 < t2; t1++) {
					...
					if (pPatternType[t1] > 0 && pPatternType[t2] > 0) {
						var zzDist = ImpedanceRoster.GetValue("distance", Constants.Mode.SOV, Constants.PathType.FULL_NETWORK, votValue, Constants.Time.EIGHT_AM, pUsualLocationZoneId[t1], pUsualLocationZoneId[t2]).Variable;
						var circuityDistance =
							(zzDist > Global.Configuration.MaximumBlendingDistance)
								? Constants.DEFAULT_VALUE
								: (Global.Configuration.DestinationScale == Constants.DestinationScale.PARCEL && Global.Configuration.UseShortDistanceNodeToNodeMeasures)
									? pUsualLocationParcel[t1].NodeToNodeDistance(pUsualLocationParcel[t2])
									: (Global.Configuration.DestinationScale == Constants.DestinationScale.PARCEL && Global.Configuration.UseShortDistanceCircuityMeasures)
										? pUsualLocationParcel[t1].CircuityDistance(pUsualLocationParcel[t2])
										: Constants.DEFAULT_VALUE;
						var skimValue =
							Global.Configuration.DestinationScale != Constants.DestinationScale.PARCEL
								? ImpedanceRoster.GetValue("ivtime", Constants.Mode.SOV, Constants.PathType.FULL_NETWORK, votValue, Constants.Time.EIGHT_AM, pUsualLocationZoneId[t1], pUsualLocationZoneId[t2])
								: ImpedanceRoster.GetValue("ivtime", Constants.Mode.SOV, Constants.PathType.FULL_NETWORK, votValue, Constants.Time.EIGHT_AM, pUsualLocationParcel[t1], pUsualLocationParcel[t2], circuityDistance);
