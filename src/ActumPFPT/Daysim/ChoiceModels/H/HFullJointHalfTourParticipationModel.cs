...
namespace Daysim.ChoiceModels.H {
	public static class HFullJointHalfTourParticipationModel {
		...
		private static void Initialize() {
			lock (_lock) {
				...
				ChoiceModelHelper.Initialize(ref _helpers[ParallelUtility.GetBatchFromThreadId()], CHOICE_MODEL_NAME,
													  Global.GetInputPath(Global.Configuration.FullJointHalfTourParticipationModelCoefficients),
													  TOTAL_ALTERNATIVES, TOTAL_NESTED_ALTERNATIVES, TOTAL_LEVELS, MAX_PARAMETER);
			}
		}

		public static int[] Run(HouseholdDayWrapper householdDay, int jHTSimulated, int genChoice, bool[,] jHTAvailable, bool[] fHTAvailable, bool[,] jHTParticipation) {
		...
		private static void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, HouseholdDayWrapper householdDay, int jHTSimulated, int genChoice, bool[,] jHTAvailable, bool[] fHTAvailable, int[][] altParticipants, int choice = Constants.DEFAULT_VALUE) {
			...
			foreach (PersonDayWrapper personDay in orderedPersonDays) {
				...
				if (count <= 5) {
					...
					pFemaleNonMandatory[count] = personDay.Person.IsAdultFemale.ToFlag() * (personDay.PatternType == Constants.PatternType.NONMANDATORY).ToFlag();
					...
					pTransitAdult[count] = (personDay.Person.TransitPassOwnershipFlag == 1 ? 1 : 0) * (personDay.Person.IsAdult.ToFlag());
					pTransitChild[count] = (personDay.Person.TransitPassOwnershipFlag == 1 ? 1 : 0) * ((!personDay.Person.IsAdult).ToFlag());
			...
			for (int alt = 0; alt < 32; alt++) {
				...
				if (available[alt] == true) {
					for (int i = 1; i <= 5; i++) {
						...
						if (pAdult[i] == 1 && (pPatternType[i] == Constants.PatternType.NONMANDATORY || pUsualLocation[i].Id == sameUsualLocation.Id)) {
							numberOfAvailableChildren++;
				...
				var nestedAlternative = HWorkTourModeModel.RunNested(pPersonDay, pPersonDay.Household.ResidenceParcel, sameUsualLocation, destinationArrivalTime, destinationDepartureTime, pPersonDay.Household.VehiclesAvailable);
				...
				//alternative.AddUtilityTerm(103, (numberOfParticipatingAdults < numberOfAvailableAdults) ? 1 : 0);
