...
namespace Daysim.ChoiceModels.H {
	public static class HJointTourParticipationModel {
		...
		private static void Initialize() {
			lock (_lock) {
				...
				ChoiceModelHelper.Initialize(ref _helpers[ParallelUtility.GetBatchFromThreadId()], CHOICE_MODEL_NAME,
													  Global.GetInputPath(Global.Configuration.JointTourParticipationModelCoefficients),
													  TOTAL_ALTERNATIVES, TOTAL_NESTED_ALTERNATIVES, TOTAL_LEVELS, MAX_PARAMETER);
			}
		}

		public static int[] Run(HouseholdDayWrapper householdDay, int nCallsForTour, int[] purpose, bool[,] jTParticipation) {
			...
			var choiceProbabilityCalculator = _helpers[ParallelUtility.GetBatchFromThreadId()].GetChoiceProbabilityCalculator((householdDay.Household.Id * 397) ^ 2 * nCallsForTour);
		...
		private static void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, HouseholdDayWrapper householdDay, int nCallsForTour, int[] purpose, int[][] altParticipants, int choice = Constants.DEFAULT_VALUE) {
			...
			int escortPurpose = purpose[nCallsForTour] == Constants.Purpose.ESCORT ? 1 : 0;
			int shopPurpose = purpose[nCallsForTour] == Constants.Purpose.SHOPPING ? 1 : 0;
			int socialPurpose = (purpose[nCallsForTour] == Constants.Purpose.SOCIAL ? 1 : 0);
			int recreationPurpose = (purpose[nCallsForTour] == Constants.Purpose.RECREATION) ? 1 : 0;
			int personalBusinessMedicalPurpose = (purpose[nCallsForTour] == Constants.Purpose.PERSONAL_BUSINESS
														 || purpose[nCallsForTour] == Constants.Purpose.MEDICAL) ? 1 : 0;
			int meal = (purpose[nCallsForTour] == Constants.Purpose.MEAL) ? 1 : 0;
			int socialRecreationPurpose = (purpose[nCallsForTour] == Constants.Purpose.SOCIAL ||
													purpose[nCallsForTour] == Constants.Purpose.RECREATION) ? 1 : 0;
			//nt recreationPurpose =	(purpose[nCallsForTour] == Constants.Purpose.RECREATION)	 ? 1 : 0;

			var votALSegment = householdDay.Household.VotALSegment;
			...
			ICondensedParcel[] pUsualLocationParcel = new ICondensedParcel[6];
			...
			foreach (PersonDayWrapper personDay in orderedPersonDays) {
				...
				if (count <= 5) {
					...
					pLessThan3NonMandatoryTours[count] = personDay.CreatedNonMandatoryTours < 3;
					pLessThan3TourPurposes[count] = personDay.TotalCreatedTourPurposes < 3;
					...
					pAdultMandatory[count] = personDay.Person.IsAdult.ToFlag() * (personDay.PatternType == Constants.PatternType.MANDATORY).ToFlag();
					pNonMandatory[count] = (personDay.PatternType == Constants.PatternType.NONMANDATORY).ToFlag();
					pAdultNonMandatory[count] = (personDay.PatternType == Constants.PatternType.NONMANDATORY).ToFlag() * personDay.Person.IsAdult.ToFlag();
					...
					pTransit[count] = (personDay.Person.TransitPassOwnershipFlag == 1 ? 1 : 0);
					pMandatory[count] = (personDay.PatternType == Constants.PatternType.MANDATORY).ToFlag();
					...
					pNonMandatoryChild[count] = (personDay.PatternType == Constants.PatternType.NONMANDATORY).ToFlag() * personDay.Person.IsChildUnder16.ToFlag();
					pMandatoryChild[count] = (personDay.PatternType == Constants.PatternType.MANDATORY).ToFlag() * personDay.Person.IsChildUnder16.ToFlag();
			...
			for (int alt = 0; alt < 32; alt++) {
				...
				for (int i = 1; i <= 5; i++) {
					if (altParticipants[alt][i] == 1 && pPatternType[i] == Constants.PatternType.HOME) {
