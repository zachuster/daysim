...
namespace Daysim.ChoiceModels {
	public static class HWorkTourModeModel {
		private const string CHOICE_MODEL_NAME = "HWorkTourModeModel";
		...
		private static readonly int[] _nestedAlternativeIds = new[] { 0, 19, 19, 20, 21, 21, 22, 22, 0 };
		private static readonly int[] _nestedAlternativeIndexes = new[] { 0, 0, 0, 1, 2, 2, 3, 3, 0 };
		private static TourWrapperCreator _creator = Global.Kernel.Get<TourWrapperCreator>();
		private static void Initialize() {
			if (_helpers[ParallelUtility.GetBatchFromThreadId()] != null) {
				return;
			}
			ChoiceModelHelper.Initialize(ref _helpers[ParallelUtility.GetBatchFromThreadId()], CHOICE_MODEL_NAME, Global.GetInputPath(Global.Configuration.WorkTourModeModelCoefficients), Constants.Mode.TOTAL_MODES, TOTAL_NESTED_ALTERNATIVES, TOTAL_LEVELS, MAX_PARAMETER);
		}
		...
		public static ChoiceProbabilityCalculator.Alternative RunNested(IPersonWrapper person, ICondensedParcel originParcel, ICondensedParcel destinationParcel, int destinationArrivalTime, int destinationDepartureTime, int householdCars) {
			if (person == null) {
		...
		public static ChoiceProbabilityCalculator.Alternative RunNested(IPersonDayWrapper personDay, ICondensedParcel originParcel, ICondensedParcel destinationParcel, int destinationArrivalTime, int destinationDepartureTime, int householdCars) {
			if (personDay == null) {
		...
		private static ChoiceProbabilityCalculator.Alternative RunNested(ITourWrapper tour, ICondensedParcel destinationParcel, int householdCars, double transitDiscountFraction) {
			Initialize();
		...
		private static void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, ITourWrapper tour, IEnumerable<PathTypeModel> pathTypeModels, ICondensedParcel destinationParcel, int householdCars, int choice = Constants.DEFAULT_VALUE) {
