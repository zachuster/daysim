...
namespace Daysim.ChoiceModels {
	public static class HWorkBasedSubtourModeModel {
		private const string CHOICE_MODEL_NAME = "HWorkBasedSubtourModeModel";
		...
		private static void Initialize() {
			...
			ChoiceModelHelper.Initialize(ref _helpers[ParallelUtility.GetBatchFromThreadId()], CHOICE_MODEL_NAME, Global.GetInputPath(Global.Configuration.WorkBasedSubtourModeModelCoefficients), Constants.Mode.TOTAL_MODES, TOTAL_NESTED_ALTERNATIVES, TOTAL_LEVELS, MAX_PARAMETER);
		}

		public static void Run(TourWrapper subtour) {
			if (subtour == null) {
		...
		public static ChoiceProbabilityCalculator.Alternative RunNested(ITourWrapper subtour, ICondensedParcel destinationParcel) {
			if (subtour == null) {
				throw new ArgumentNullException("subtour");
			}
		...
		private static void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, ITourWrapper subtour, IEnumerable<PathTypeModel> pathTypeModels, ICondensedParcel destinationParcel, int parentTourMode, int choice = Constants.DEFAULT_VALUE) {
