...
namespace Daysim.ChoiceModels {
	public static class HWorkTourDestinationModel {
		private const string CHOICE_MODEL_NAME = "HWorkTourDestinationModel";
		...
		private static void Initialize(int sampleSize) {
			...
			ChoiceModelHelper.Initialize(ref _helpers[ParallelUtility.GetBatchFromThreadId()], CHOICE_MODEL_NAME, Global.Configuration.WorkTourDestinationModelCoefficients, sampleSize + 1, TOTAL_NESTED_ALTERNATIVES, TOTAL_LEVELS, MAX_PARAMETER);
		}

		public static void Run(TourWrapper tour, HouseholdDayWrapper householdDay, int sampleSize) {
			if (tour == null) {
...
//					: 1 / (Math.Max(totalAvailableMinutes, 1) / 60D);
		...
		private sealed class TourDestinationUtilities : ISamplingUtilities {
			...
			public TourDestinationUtilities(TourWrapper tour, int sampleSize, int primaryFlag, int secondaryFlag, int fastestAvailableTimeOfDay, int maxAvailableMinutes) {
				_tour = tour;
...
//				var householdHasChildren = household.HasChildren;
				...
				if (_tour.IsHomeBasedTour) {
					var nestedAlternative = HWorkTourModeModel.RunNested(personDay, _tour.OriginParcel, destinationParcel, _tour.DestinationArrivalTime, _tour.DestinationDepartureTime, household.VehiclesAvailable);
