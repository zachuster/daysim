...
namespace Daysim.ChoiceModels {
	public static class HPayToParkAtWorkplaceModel {
		...
		private static void Initialize() {
			...
			ChoiceModelHelper.Initialize(ref _helpers[ParallelUtility.GetBatchFromThreadId()], CHOICE_MODEL_NAME, Global.GetInputPath(Global.Configuration.PayToParkAtWorkplaceModelCoefficients), TOTAL_ALTERNATIVES, TOTAL_NESTED_ALTERNATIVES, TOTAL_LEVELS, MAX_PARAMETER);
		}
		...
		public static void Run(IPersonWrapper person) {
			...
			if (_helpers[ParallelUtility.GetBatchFromThreadId()].ModelIsInEstimationMode) {
				if (person.PayToParkAtWorkplaceFlag < 0 || person.PayToParkAtWorkplaceFlag > 1) {
					return;
				}

				RunModel(choiceProbabilityCalculator, person, person.PayToParkAtWorkplaceFlag);
				...
				person.PayToParkAtWorkplaceFlag = choice;
			}
		}
