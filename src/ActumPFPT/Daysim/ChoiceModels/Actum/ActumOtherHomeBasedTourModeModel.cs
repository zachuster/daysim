...
namespace Daysim.ChoiceModels {
	public class ActumOtherHomeBasedTourModeModel : ChoiceModel {
		...
		public void Run(TourWrapper tour) {
			...
			Initialize(CHOICE_MODEL_NAME, Global.GetInputPath(Global.Configuration.OtherHomeBasedTourModeModelCoefficients), Constants.Mode.TOTAL_MODES, TOTAL_NESTED_ALTERNATIVES, TOTAL_LEVELS, MAX_PARAMETER);
		...
		public ChoiceProbabilityCalculator.Alternative RunNested(TourWrapper tour, CondensedParcel destinationParcel) {
			if (tour == null) {
				throw new ArgumentNullException("tour");
			}

			Initialize(CHOICE_MODEL_NAME, Global.Configuration.OtherHomeBasedTourModeModelCoefficients, Constants.Mode.TOTAL_MODES, TOTAL_NESTED_ALTERNATIVES, TOTAL_LEVELS, MAX_PARAMETER);
