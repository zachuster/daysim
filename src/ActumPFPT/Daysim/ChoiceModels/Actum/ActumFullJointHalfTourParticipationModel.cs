...
namespace Daysim.ChoiceModels.Actum {
	public class ActumFullJointHalfTourParticipationModel : ChoiceModel {
		...
		public int[] Run(ActumHouseholdDayWrapper householdDay, int jHTSimulated, int genChoice, bool[,] jHTAvailable, bool[] fHTAvailable, bool[,] jHTParticipation) {
			...
			var choiceProbabilityCalculator = _helpers[ParallelUtility.GetBatchFromThreadId()].GetChoiceProbabilityCalculator((householdDay.Household.Id * 397) ^ 2 * jHTSimulated);
		...
		private void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, ActumHouseholdDayWrapper householdDay, int jHTSimulated, int genChoice, bool[,] jHTAvailable, bool[] fHTAvailable, int[][] altParticipants, int choice = Constants.DEFAULT_VALUE) {
			...
			ICondensedParcel[] pUsualLocation = new ICondensedParcel[6];
			...
			foreach (ActumPersonDayWrapper personDay in orderedPersonDays) {
				...
				if (count <= 5) {
					...
					pAdultNonMandatory[count] = (personDay.Person.IsAdult && personDay.PatternType == Constants.PatternType.NONMANDATORY).ToFlag();
					...
					pType8Mandatory[count] = (personDay.Person.IsChildUnder5 && personDay.PatternType == Constants.PatternType.MANDATORY).ToFlag();
					pType8NonMandatory[count] = (personDay.Person.IsChildUnder5 && personDay.PatternType == Constants.PatternType.NONMANDATORY).ToFlag();

					pType7Mandatory[count] = (personDay.Person.IsChildAge5Through15 && personDay.PatternType == Constants.PatternType.MANDATORY).ToFlag();
					pType7NonMandatory[count] = (personDay.Person.IsChildAge5Through15 && personDay.PatternType == Constants.PatternType.NONMANDATORY).ToFlag();

					pYouthMandatory[count] = (!personDay.Person.IsChildUnder5 && !personDay.Person.IsAdult && personDay.PatternType == Constants.PatternType.MANDATORY).ToFlag();
					pYouthNonMandatory[count] = (!personDay.Person.IsChildUnder5 && !personDay.Person.IsAdult && personDay.PatternType == Constants.PatternType.NONMANDATORY).ToFlag();
					pYouth[count] = (!personDay.Person.IsChildUnder5 && !personDay.Person.IsAdult).ToFlag();

					pAdultMandatory[count] = (personDay.Person.IsAdult && personDay.PatternType == Constants.PatternType.MANDATORY).ToFlag();
			...
			for (int alt = 0; alt < 32; alt++) {
				...
				ICondensedParcel sameUsualLocation = householdDay.Household.ResidenceParcel;
				for (int i = 1; i <= 5; i++) {
				...
				var nestedAlternative = (Global.ChoiceModelDictionary.Get("ActumWorkTourModeModel") as ActumWorkTourModeModel).RunNested(pPersonDay, pPersonDay.Household.ResidenceParcel, sameUsualLocation, destinationArrivalTime, destinationDepartureTime, pPersonDay.Household.VehiclesAvailable);
