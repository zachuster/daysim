...
namespace Daysim.ChoiceModels.Actum {
	public class ActumWorkBasedSubtourGenerationModel : ChoiceModel {
		...
		public int Run(ActumTourWrapper tour, ActumHouseholdDayWrapper householdDay, int nCallsForTour, int choice = Constants.Purpose.NONE_OR_HOME) {
			if (tour == null) {
			...
			if (Global.Configuration.IsInEstimationMode) {
				if (!(choice == Constants.Purpose.NONE_OR_HOME)) {  // simplifying choice.  TODO:  will need to deal with personal business subtours distinctly from home based in tour models
		...
		private void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, ActumTourWrapper tour, ActumHouseholdDayWrapper householdDay, int nCallsForTour, int choice = Constants.DEFAULT_VALUE) {
			var person = (ActumPersonWrapper) tour.Person;
			var personDay = (ActumPersonDayWrapper) tour.PersonDay;
			...
			var workAggregateLogsum = Global.AggregateLogsums[tour.DestinationParcel.ZoneId][Constants.Purpose.WORK_BASED][carOwnership][votALSegment][workTaSegment];
			...
			alternative = choiceProbabilityCalculator.GetAlternative(1, true, choice == Constants.Purpose.PERSONAL_BUSINESS);
