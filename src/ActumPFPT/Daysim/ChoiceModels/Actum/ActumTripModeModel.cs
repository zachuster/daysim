...
{
	public class ActumTripModeModel : ChoiceModel
	{
		...
		public void Run(ActumHouseholdDayWrapper householdDay, ActumTripWrapper trip)
		{
			...
			Initialize(CHOICE_MODEL_NAME, Global.Configuration.TripModeModelCoefficients, Constants.Mode.TOTAL_MODES,
			...
			{
				if (destinationParcel == null || originParcel == null || trip.Mode <= Constants.Mode.NONE ||
		...
		private void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, ActumTripWrapper trip,
		                      List<PathTypeModel> pathTypeModels, ICondensedParcel originParcel,
		                      ICondensedParcel destinationParcel,
		                      int choice = Constants.DEFAULT_VALUE)
		{
			...
			var PTpass = person.TransitPassOwnershipFlag;
			...
			var homeBasedNotWorkSchoolEscortTourFlag =
				(tour.IsHomeBasedTour && tour.DestinationPurpose > Constants.Purpose.ESCORT).ToFlag();
			...
			var amPeriodFlag = departureTime.IsLeftExclusiveBetween(Constants.Time.SIX_AM, Constants.Time.NINE_AM).ToFlag();
				//GV changed to 6-9 am
			var middayPeriodFlag = departureTime.IsLeftExclusiveBetween(Constants.Time.NINE_AM, Constants.Time.THREE_PM).ToFlag();
			var pmPeriodFlag = departureTime.IsLeftExclusiveBetween(Constants.Time.THREE_PM, Constants.Time.SIX_PM).ToFlag();
				//GV changed to 3-6 pm
			var eveningPeriodFlag = (departureTime > Constants.Time.SIX_PM).ToFlag(); //GV changed to 6 pm
			...
			if (person.Age < 18)
			{
				tripModeAvailable[Constants.Mode.SOV] = false;
				tripModeAvailable[Constants.Mode.HOVDRIVER] = false;
			}
			...
			foreach (var pathTypeModel in pathTypeModels)
			{
				...
				switch (mode)
				{
					...
					case Constants.Mode.HOVPASSENGER:
						...
						// Car Driver mode where there is a passenger 
