...
namespace Daysim.ChoiceModels {
	public class ActumTourModeTimeModel : ChoiceModel {
		...
		public void Run(ActumHouseholdDayWrapper householdDay, ActumTourWrapper tour,
		...
		private void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, ActumHouseholdDayWrapper householdDay, ActumTourWrapper tour,
			...
			for (var mode = Constants.Mode.WALK; mode <= Constants.Mode.SCHOOL_BUS; mode++) {
				...
				switch (mode) {
					...
					//Car Deriver in a car where a passenger is present (or more)
			...
			foreach (var modeTimes in HTourModeTime.ModeTimes) {
				...
				bool available = modeTimes.LongestFeasibleWindow != null
					...
					&& (person.Age >= 18 || (modeTimes.Mode != Constants.Mode.SOV && modeTimes.Mode != Constants.Mode.HOVDRIVER));
				...
				alternative.AddNestedAlternative(HTourModeTime.TOTAL_TOUR_MODE_TIMES + mode, mode - 1, THETA_PARAMETER);
