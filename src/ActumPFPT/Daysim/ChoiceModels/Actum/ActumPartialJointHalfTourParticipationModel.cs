...
namespace Daysim.ChoiceModels.Actum {
	public class ActumPartialJointHalfTourParticipationModel : ChoiceModel {
		...
		public int[] Run(ActumHouseholdDayWrapper householdDay, int jHTSimulated, int genChoice, bool[,] jHTAvailable, bool[] pHTAvailable, bool[,] jHTParticipation) {
		...
		private void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, ActumHouseholdDayWrapper householdDay, int jHTSimulated, int genChoice, bool[,] jHTAvailable, bool[] pHTAvailable, int[][] altParticipants, int choice = Constants.DEFAULT_VALUE) {
			...
			for (int alt = 0; alt < 32; alt++) {
				...
				for (int i = 1; i <= 5; i++) {
					if (altParticipants[alt][i] == 1 && !(pPatternType[i] == Constants.PatternType.MANDATORY)) {
