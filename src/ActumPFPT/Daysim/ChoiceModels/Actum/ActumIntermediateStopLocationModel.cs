...
namespace Daysim.ChoiceModels {
	public class ActumIntermediateStopLocationModel : ChoiceModel {
		...
		public void Run(ActumTripWrapper trip, ActumHouseholdDayWrapper householdDay, int sampleSize) {
			if (trip == null) {
		...
		private void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, ActumTripWrapper trip, ActumHouseholdDayWrapper householdDay, int sampleSize, ICondensedParcel choice = null) {
		...
		private sealed class IntermediateStopLocationUtilities : ISamplingUtilities {
			...
			private readonly ActumTripWrapper _trip;
			//#if TRACE
			...
			public IntermediateStopLocationUtilities(ActumTripWrapper trip, int sampleSize, int destinationDepartureTime, int availableWindow) {
				_trip = trip;
				//#if TRACE
				//				_choice = choice;
				//#endif
			...
			public void SetUtilities(ISampleItem sampleItem, int sampleFrequency) {
				...
				var household = (ActumHouseholdWrapper) _trip.Household;
				var person = (ActumPersonWrapper) _trip.Person;
				var tour = (ActumTourWrapper) _trip.Tour;
				...
				switch (tour.Mode) {
					...
					case Constants.Mode.HOVPASSENGER:
						if (wdis1 < WTHRESH && wdis2 < WTHRESH && wdis1 > Constants.EPSILON && wdis2 > Constants.EPSILON) {
							...
							if (_trip.Direction == Constants.TourDirection.ORIGIN_TO_DESTINATION) {
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.HOVDRIVER, purpose, costCoef, timeCoef, transitDiscountFraction, 1, tour.OriginParcel, destinationParcel, _trip.OriginParcel, out d2Dis1, out d2Time1, out gd2Time1, _destinationDepartureTime, parkingTime);
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.HOVDRIVER, purpose, costCoef, timeCoef, transitDiscountFraction, 2, tour.OriginParcel, destinationParcel, _trip.OriginParcel, out d2Dis2, out d2Time2, out gd2Time2, _destinationDepartureTime, parkingTime);
							}
							else {
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.HOVDRIVER, purpose, costCoef, timeCoef, transitDiscountFraction, 1, _trip.OriginParcel, destinationParcel, tour.OriginParcel, out d2Dis1, out d2Time1, out gd2Time1, _destinationDepartureTime, parkingTime);
								GetGenTime(_trip.Household.RandomUtility, Constants.Mode.HOVDRIVER, purpose, costCoef, timeCoef, transitDiscountFraction, 2, _trip.OriginParcel, destinationParcel, tour.OriginParcel, out d2Dis2, out d2Time2, out gd2Time2, _destinationDepartureTime, parkingTime);
							}
