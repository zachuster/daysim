...
namespace Daysim.ChoiceModels {
	public class IntermediateStopGenerationModel : ChoiceModel {
		...
		public int Run(ITripWrapper trip, int choice = Constants.Purpose.NONE_OR_HOME) {
			if (trip == null) {
...
//			var simulatedSocialStopsFlag = personDay.HasSimulatedSocialStops.ToFlag();
			...
			var timeWindow =
				tour.IsHomeBasedTour
					? personDay.TimeWindow.AvailableWindow(startTime, Constants.TimeDirection.BOTH)
					: tour.ParentTour.TimeWindow.AvailableWindow(startTime, Constants.TimeDirection.BOTH);
			...
			if (personDay.WorkStops > 0 && tour.DestinationPurpose <= Constants.Purpose.SCHOOL) {
...
//				alternative.AddUtility(75, socialOrRecreationTourFlag);
			...
			if (personDay.SchoolStops > 0 && tour.DestinationPurpose <= Constants.Purpose.SCHOOL) {
...
//				alternative.AddUtility(97, simulatedSchoolStopsFlag);
			...
			if (personDay.EscortStops > 0 && tour.DestinationPurpose <= Constants.Purpose.ESCORT) {
