...
namespace Daysim.Factories {
	public class TripWrapperCreator  : ITripWrapperCreator
	{
		#region ITripWrapperCreator Members

		public ITripWrapper CreateWrapper(ITrip trip, ITourWrapper tour, IHalfTour halfTour) {
			return new TripWrapper(trip, tour, halfTour);
		}

		public ITripWrapper CreateWrapperWithTrip(ITourWrapper t, ITour tour, ITour wrapperTour, int nextTripId, int direction, 
			int sequence, bool isToTourOrigin, IHalfTour halfTour )
		{
			ITripWrapper tw = CreateWrapper(new Trip {
					Id = nextTripId,
		...
		{
			ITripWrapper tw = Global.Kernel.Get<TripWrapperFactory>().TripWrapperCreator.CreateWrapper(new Trip {
					Id = nextTripId,
					TourId = wrapperTour.Id,
					HouseholdId = wrapperTour.HouseholdId,
					PersonSequence = wrapperTour.PersonSequence,
