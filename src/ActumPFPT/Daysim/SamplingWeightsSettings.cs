	...
	public class SamplingWeightsSettings : ISamplingWeightsSettings {
		...
		public int GetTourDestinationSegment(int purpose, int priority, int mode, int personType)
		{
			if ((purpose == Constants.Purpose.WORK || purpose == Constants.Purpose.BUSINESS) &&
			    priority == Constants.TourPriority.WORK_BASED_TOUR && mode >= Constants.Mode.SOV)
			{
				return 0;
			}


			if (purpose == Constants.Purpose.SCHOOL && priority == Constants.TourPriority.WORK_BASED_TOUR &&
			    mode >= Constants.Mode.SOV)
			{
				return 1;
			}

			if (purpose == Constants.Purpose.ESCORT && priority == Constants.TourPriority.WORK_BASED_TOUR &&
			    mode >= Constants.Mode.SOV)
			{
				return 2;
			}

			if (purpose == Constants.Purpose.PERSONAL_BUSINESS && priority == Constants.TourPriority.WORK_BASED_TOUR &&
			    mode >= Constants.Mode.SOV)
			{
				return 3;
			}

			if (purpose == Constants.Purpose.SHOPPING && priority == Constants.TourPriority.WORK_BASED_TOUR &&
			    mode >= Constants.Mode.SOV)
			{
				return 4;
			}

			if (purpose == Constants.Purpose.MEAL && priority == Constants.TourPriority.WORK_BASED_TOUR &&
			    mode >= Constants.Mode.SOV)
			{
				return 5;
			}

			if (purpose == Constants.Purpose.SOCIAL && priority == Constants.TourPriority.WORK_BASED_TOUR &&
			    mode >= Constants.Mode.SOV)
			{
				return 6;
			}

			if (purpose == Constants.Purpose.RECREATION && priority == Constants.TourPriority.WORK_BASED_TOUR &&
			    mode >= Constants.Mode.SOV)
			{
				return 7;
			}

			if (purpose == Constants.Purpose.MEDICAL && priority == Constants.TourPriority.WORK_BASED_TOUR &&
			    mode >= Constants.Mode.SOV)
			{
				return 8;
			}
			...
			if (purpose == Constants.Purpose.SCHOOL && mode >= Constants.Mode.SOV &&
			    priority == Constants.TourPriority.USUAL_LOCATION && personType <= Constants.PersonType.UNIVERSITY_STUDENT)
			{
				return 36;
			}
			...
			if ((purpose == Constants.Purpose.WORK || purpose == Constants.Purpose.BUSINESS) && mode == Constants.Mode.WALK)
			{
				return 18;
			}

			if (purpose == Constants.Purpose.SCHOOL && mode == Constants.Mode.WALK)
			{
				return 19;
			}

			if (purpose == Constants.Purpose.ESCORT && mode == Constants.Mode.WALK)
			{
				return 20;
			}

			if (purpose == Constants.Purpose.PERSONAL_BUSINESS && mode == Constants.Mode.WALK)
			{
				return 21;
			}

			if (purpose == Constants.Purpose.SHOPPING && mode == Constants.Mode.WALK)
			{
				return 22;
			}

			if (purpose == Constants.Purpose.MEAL && mode == Constants.Mode.WALK)
			{
				return 23;
			}

			if (purpose == Constants.Purpose.SOCIAL && mode == Constants.Mode.WALK)
			{
				return 24;
			}

			if (purpose == Constants.Purpose.RECREATION && mode == Constants.Mode.WALK)
			{
				return 25;
			}

			if (purpose == Constants.Purpose.MEDICAL && mode == Constants.Mode.WALK)
			{
				return 26;
			}

			if ((purpose == Constants.Purpose.WORK || purpose == Constants.Purpose.BUSINESS) && mode == Constants.Mode.BIKE)
			{
				return 27;
			}

			if (purpose == Constants.Purpose.SCHOOL && mode == Constants.Mode.BIKE)
			{
				return 28;
			}

			if (purpose == Constants.Purpose.ESCORT && mode == Constants.Mode.BIKE)
			{
				return 29;
			}

			if (purpose == Constants.Purpose.PERSONAL_BUSINESS && mode == Constants.Mode.BIKE)
			{
				return 30;
			}

			if (purpose == Constants.Purpose.SHOPPING && mode == Constants.Mode.BIKE)
			{
				return 31;
			}

			if (purpose == Constants.Purpose.MEAL && mode == Constants.Mode.BIKE)
			{
				return 32;
			}

			if (purpose == Constants.Purpose.SOCIAL && mode == Constants.Mode.BIKE)
			{
				return 33;
			}

			if (purpose == Constants.Purpose.RECREATION && mode == Constants.Mode.BIKE)
			{
				return 34;
			}

			if (purpose == Constants.Purpose.MEDICAL && mode == Constants.Mode.BIKE)
			{
				return 35;
			}

			if (purpose == Constants.Purpose.BUSINESS && priority == Constants.TourPriority.WORK_BASED_TOUR &&
			    mode >= Constants.Mode.SOV)
			{
				return 0;
			}
