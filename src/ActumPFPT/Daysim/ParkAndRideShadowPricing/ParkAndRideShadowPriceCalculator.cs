			...
			using (var shadowPriceWriter = new ParkAndRideShadowPriceWriter(new FileInfo(Global.ParkAndRideShadowPricesPath))) {
				foreach (var node in ChoiceModelFactory.ParkAndRideNodeDao.Nodes) {
					...
					for (var i = 1; i < Constants.Time.MINUTES_IN_A_DAY; i++) {
						double shadowPrice;
						double maxLoad = 0;
						for (var j = Math.Max(i - (Global.Configuration.ParkAndRideShadowPriceTimeSpread), 1); j < Math.Min(i + (Global.Configuration.ParkAndRideShadowPriceTimeSpread), Constants.Time.MINUTES_IN_A_DAY); j++) {
