...
namespace Daysim.ParkAndRideShadowPricing {
	public sealed class ParkAndRideShadowPriceWriter : IDisposable {
		...
		public ParkAndRideShadowPriceWriter(FileInfo file) {
			...
			_writer.Write("PRLOAD" + string.Format("{0:0000}", Constants.Time.MINUTES_IN_A_DAY - 1));
			_writer.WriteLine();
		}
		...
		public void Write(ParkAndRideNodeWrapper node) {
			if (node == null) {
			...
			for (var i = 1; i <= Constants.Time.MINUTES_IN_A_DAY - 1; i++) {
				_writer.Write(string.Format("{0:0.000000}{1}", node.ParkAndRideLoad[i - 1], Global.Configuration.ParkAndRideShadowPriceDelimiter));
			}
			_writer.Write(string.Format("{0:0.000000}", node.ParkAndRideLoad[Constants.Time.MINUTES_IN_A_DAY - 1]));
			_writer.WriteLine();
		}
	}
}