...
namespace Daysim {
	public sealed class AggregateLogsumsCalculator {
		#region fields
		...
		private readonly Dictionary<int, Zone> _eligibleZones;
		private readonly Subzone[][] _zoneSubzones;
		private readonly int _middayStartMinute;

		#endregion

		private AggregateLogsumsCalculator() {
		...
		public static void Calculate(RandomUtility randomUtility) {
			...
			var aggregateLogsumCalculator = new AggregateLogsumsCalculator();
			var zoneCount = aggregateLogsumCalculator._zoneCount;

			Global.AggregateLogsums = new double[zoneCount][][][][];

			Parallel.For(0, zoneCount, new ParallelOptions {MaxDegreeOfParallelism = Global.LargeDegreeOfParallelism}, id => CalculateZone(randomUtility, aggregateLogsumCalculator, id));
		...
		private static void CalculateZone(RandomUtility randomUtility, AggregateLogsumsCalculator aggregateLogsumsCalculator, int id) {
			Global.AggregateLogsums[id] = aggregateLogsumsCalculator.ComputeZone(randomUtility, id);
		}

		private static double[][][][][] LoadAggregateLogsumsFromFile(FileInfo file) {
		...
		private static void SaveAggregateLogsumsToFile(FileInfo file) {
		...
		private double[][][][] ComputeZone(RandomUtility randomUtility, int id) {
			var purposes = new double[Constants.Purpose.TOTAL_PURPOSES][][][];

			for (var purpose = Constants.Purpose.HOME_BASED_COMPOSITE; purpose <= Constants.Purpose.SOCIAL; purpose++) {
				var carOwnerships = new double[Constants.CarOwnership.TOTAL_CAR_OWNERSHIPS][][];

				purposes[purpose] = carOwnerships;

				for (var carOwnership = Constants.CarOwnership.CHILD; carOwnership < Constants.CarOwnership.TOTAL_CAR_OWNERSHIPS; carOwnership++) {
					var votALSegments = new double[Constants.VotALSegment.TOTAL_VOT_ALSEGMENTS][];

					carOwnerships[carOwnership] = votALSegments;

					for (var votALSegment = Constants.VotALSegment.LOW; votALSegment < Constants.VotALSegment.TOTAL_VOT_ALSEGMENTS; votALSegment++) {
						var transitAccesses = new double[Constants.TransitAccess.TOTAL_TRANSIT_ACCESSES];
			...
			foreach (var destination in _eligibleZones.Values) {
				...
				// mode impedance
				var sovInVehicleTimeFromOrigin = ImpedanceRoster.GetValue("ivtime", Constants.Mode.SOV, Constants.PathType.FULL_NETWORK, Constants.ValueOfTime.DEFAULT_VOT, _middayStartMinute, id, destination.Id).Variable;
				...
				for (var purpose = Constants.Purpose.HOME_BASED_COMPOSITE; purpose <= Constants.Purpose.SOCIAL; purpose++) {
					...
					// set purpose inputs
					var escortFlag = (purpose == Constants.Purpose.ESCORT).ToFlag();
					var personalBusinessFlag = (purpose == Constants.Purpose.PERSONAL_BUSINESS).ToFlag();
					var shoppingFlag = (purpose == Constants.Purpose.SHOPPING).ToFlag();
					var mealFlag = (purpose == Constants.Purpose.MEAL).ToFlag();
					var socialFlag = (purpose == Constants.Purpose.SOCIAL).ToFlag();
					...
					for (var carOwnership = Constants.CarOwnership.CHILD; carOwnership < Constants.CarOwnership.TOTAL_CAR_OWNERSHIPS; carOwnership++) {
						...
						var childFlag = GetChildFlag(carOwnership);
						var noCarsFlag = GetNoCarsFlag(carOwnership);
						var carCompetitionFlag = GetCarCompetitionFlag(carOwnership);
						var noCarCompetitionFlag = GetNoCarCompetitionFlag(carOwnership);
						var carDeficitFlag = GetCarDeficitFlag(carOwnership);
						...
						for (var votALSegment = Constants.VotALSegment.LOW; votALSegment < Constants.VotALSegment.TOTAL_VOT_ALSEGMENTS; votALSegment++) {
							...
							var timeCoefficient = Constants.VotALSegment.TIME_COEFFICIENT;
							var costCoefficient = (votALSegment == Constants.VotALSegment.LOW)
							                      	? Constants.VotALSegment.COST_COEFFICIENT_LOW
							                      	: (votALSegment == Constants.VotALSegment.MEDIUM)
							                      	  	? Constants.VotALSegment.COST_COEFFICIENT_MEDIUM
							                      	  	: Constants.VotALSegment.COST_COEFFICIENT_HIGH;

							for (var transitAccess = Constants.TransitAccess.GT_0_AND_LTE_QTR_MI; transitAccess < Constants.TransitAccess.TOTAL_TRANSIT_ACCESSES; transitAccess++) {
								var purposeUtility = 0D;

								// set transit access flags
								var hasNearTransitAccessFlag = (transitAccess == Constants.TransitAccess.GT_0_AND_LTE_QTR_MI).ToFlag();
								var hasNoTransitAccessFlag = (transitAccess == Constants.TransitAccess.NONE).ToFlag();
								...
								foreach (var subzone in subzones) {
									...
									if (setImpedance) {
										...
										var walkPath = PathTypeModel.Run(randomUtility, id, destination.Id, _middayStartMinute, _middayStartMinute, Constants.Purpose.PERSONAL_BUSINESS,
											costCoefficient, timeCoefficient, true, 1, 0.0, false, Constants.Mode.WALK).First();

										walkGenTime = walkPath.GeneralizedTimeLogsum;

										var sovPath = PathTypeModel.Run(randomUtility, id, destination.Id, _middayStartMinute, _middayStartMinute, Constants.Purpose.PERSONAL_BUSINESS,
											costCoefficient, timeCoefficient, true, 1, 0.0, false, Constants.Mode.SOV).First();

										// var sovDistanceFromOrigin = sovPath.PathDistance / 2D;  
										// JLBscale replaced above with following:
										var sovDistanceFromOrigin = (sovPath.PathDistance/ Global.DistanceUnitsPerMile) / 2D;
										scaledSovDistanceFromOrigin = sovDistanceFromOrigin / 10D;

										sovGenTime = sovPath.GeneralizedTimeLogsum;

										var hov2Path = PathTypeModel.Run(randomUtility, id, destination.Id, _middayStartMinute, _middayStartMinute, Constants.Purpose.PERSONAL_BUSINESS,
											costCoefficient, timeCoefficient, true, 1, 0.0, false, Constants.Mode.HOV2).First();

										hov2GenTime = hov2Path.GeneralizedTimeLogsum;

										var transitPath = PathTypeModel.Run(randomUtility, id, destination.Id, _middayStartMinute, _middayStartMinute, Constants.Purpose.PERSONAL_BUSINESS,
											costCoefficient, timeCoefficient, true, 1, Global.Configuration.Policy_UniversalTransitFareDiscountFraction, false, Constants.Mode.TRANSIT).First();
									...
									// SOV
									if (childFlag == 0 && noCarsFlag == 0 && sovGenTime != Constants.GENERALIZED_TIME_UNAVAILABLE) {
									...
									// HOV
									if (hov2GenTime != Constants.GENERALIZED_TIME_UNAVAILABLE) {
										modeUtilitySum += ComputeUtility(
									...
									// TRANSIT
									if (transitGenTime != Constants.GENERALIZED_TIME_UNAVAILABLE && hasNoTransitAccessFlag == 0 && hasNoTransitEgressFlag == 0) {
										modeUtilitySum += ComputeUtility(
											//p01 * transitFare +
									...
									// WALK
									if (walkGenTime != Constants.GENERALIZED_TIME_UNAVAILABLE) {
										modeUtilitySum += ComputeUtility(
											//p03 * walkDistance * 20);
		...
		private Subzone[][] CalculateZoneSubzones() {
			var zoneSubzones = new Subzone[_zoneCount][];

			for (var id = 0; id < _zoneCount; id++) {
				var subzones = new Subzone[TOTAL_SUBZONES];

				zoneSubzones[id] = subzones;

				for (var subzone = 0; subzone < TOTAL_SUBZONES; subzone++) {
					subzones[subzone] = new Subzone(subzone);
				}
			}
			...
			foreach (var parcel in parcelReader) {
				var subzones = zoneSubzones[parcel.ZoneId];
				// var subzone = (parcel.GetDistanceToTransit() > 0 && parcel.GetDistanceToTransit() <= .5) ? 0 : 1;  
				// JLBscale replaced above with following:
				var subzone = (parcel.GetDistanceToTransit() / Global.DistanceUnitsPerMile > 0 && parcel.GetDistanceToTransit() / Global.DistanceUnitsPerMile <= .5) ? 0 : 1;

				subzones[subzone].Households += parcel.Households;
				subzones[subzone].StudentsK8 += parcel.StudentsK8;
				subzones[subzone].StudentsHighSchool += parcel.StudentsHighSchool;
				subzones[subzone].StudentsUniversity += parcel.StudentsUniversity;
				subzones[subzone].EmploymentEducation += parcel.EmploymentEducation;
				subzones[subzone].EmploymentFood += parcel.EmploymentFood;
				subzones[subzone].EmploymentGovernment += parcel.EmploymentGovernment;
				subzones[subzone].EmploymentIndustrial += parcel.EmploymentIndustrial;
				subzones[subzone].EmploymentMedical += parcel.EmploymentMedical;
				subzones[subzone].EmploymentOffice += parcel.EmploymentOffice;
				subzones[subzone].EmploymentRetail += parcel.EmploymentRetail;
				subzones[subzone].EmploymentService += parcel.EmploymentService;
				subzones[subzone].EmploymentTotal += parcel.EmploymentTotal;
				subzones[subzone].ParkingOffStreetPaidDailySpaces += parcel.ParkingOffStreetPaidDailySpaces;
				subzones[subzone].ParkingOffStreetPaidHourlySpaces += parcel.ParkingOffStreetPaidHourlySpaces;
			}
			...
			foreach (var subzones in _eligibleZones.Values.Select(zone => zoneSubzones[zone.Id])) {
				for (var subzone = 0; subzone < TOTAL_SUBZONES; subzone++) {
					...
					subzones[subzone].SetSize(Constants.Purpose.HOME_BASED_COMPOSITE, ComputeSize(Math.Exp(HBG019) * edu + Math.Exp(HBG020) * foo + Math.Exp(HBG021) * gov + Math.Exp(HBG022) * off + Math.Exp(HBG023) * oth + Math.Exp(HBG024) * ret + Math.Exp(HBG025) * ser + Math.Exp(HBG026) * med + Math.Exp(HBG027) * ind + Math.Exp(HBG029) * hou + Math.Exp(HBG030) * uni + Math.Exp(HBG031) * k12));
					subzones[subzone].SetSize(Constants.Purpose.WORK_BASED, ComputeSize(Math.Exp(WBG019) * edu + Math.Exp(WBG020) * foo + Math.Exp(WBG021) * gov + Math.Exp(WBG022) * off + Math.Exp(WBG023) * oth + Math.Exp(WBG024) * ret + Math.Exp(WBG025) * ser + Math.Exp(WBG026) * med + Math.Exp(WBG027) * ind + Math.Exp(WBG029) * hou + Math.Exp(WBG030) * uni));
					subzones[subzone].SetSize(Constants.Purpose.ESCORT, ComputeSize(Math.Exp(PSG067) * edu + Math.Exp(PSG068) * foo + Math.Exp(PSG069) * gov + Math.Exp(PSG070) * off + Math.Exp(PSG071) * oth + Math.Exp(PSG072) * ret + Math.Exp(PSG073) * ser + Math.Exp(PSG074) * med + Math.Exp(PSG075) * ind + Math.Exp(PSG077) * hou + Math.Exp(PSG078) * uni + k12));
					subzones[subzone].SetSize(Constants.Purpose.PERSONAL_BUSINESS, ComputeSize(Math.Exp(PSG080) * edu + Math.Exp(PSG081) * foo + Math.Exp(PSG082) * gov + Math.Exp(PSG083) * off + Math.Exp(PSG084) * oth + Math.Exp(PSG085) * ret + Math.Exp(PSG086) * ser + Math.Exp(PSG087) * med + Math.Exp(PSG088) * ind + Math.Exp(PSG090) * hou + Math.Exp(PSG091) * uni));
					subzones[subzone].SetSize(Constants.Purpose.SHOPPING, ComputeSize(Math.Exp(PSG094) * foo + Math.Exp(PSG095) * gov + Math.Exp(PSG097) * oth + Math.Exp(PSG098) * ret + Math.Exp(PSG099) * ser + Math.Exp(PSG100) * med + Math.Exp(PSG103) * hou + Math.Exp(PSG105) * k12));
					subzones[subzone].SetSize(Constants.Purpose.MEAL, ComputeSize(Math.Exp(PSG107) * foo + Math.Exp(PSG115) * tot + Math.Exp(PSG117) * uni));
					subzones[subzone].SetSize(Constants.Purpose.SOCIAL, ComputeSize(Math.Exp(PSG119) * edu + Math.Exp(PSG120) * foo + Math.Exp(PSG121) * gov + Math.Exp(PSG122) * off + Math.Exp(PSG123) * oth + Math.Exp(PSG125) * ser + Math.Exp(PSG126) * med + Math.Exp(PSG129) * hou + Math.Exp(PSG130) * uni));
				}
			}
		...
		private static double ComputeSize(double size) {
		...
		private static double ComputeUtility(double utility) {
		...
		private static int GetChildFlag(int carOwnership) {
			return (carOwnership == Constants.CarOwnership.CHILD).ToFlag();
		}

		public static int GetNoCarsFlag(int carOwnership) {
			return (carOwnership == Constants.CarOwnership.NO_CARS).ToFlag();
		}

		public static int GetCarCompetitionFlag(int carOwnership) {
			return (carOwnership == Constants.CarOwnership.LT_ONE_CAR_PER_ADULT).ToFlag();
		}

		public static int GetNoCarCompetitionFlag(int carOwnership) {
			return (carOwnership == Constants.CarOwnership.ONE_OR_MORE_CARS_PER_ADULT).ToFlag();
		}

		public static int GetCarDeficitFlag(int carOwnership) {
			return (carOwnership == Constants.CarOwnership.NO_CARS || carOwnership == Constants.CarOwnership.LT_ONE_CAR_PER_ADULT).ToFlag();
		}
	}
}