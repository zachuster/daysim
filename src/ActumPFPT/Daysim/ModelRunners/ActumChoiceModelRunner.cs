...
namespace Daysim.ModelRunners {
	public sealed class ActumChoiceModelRunner : IChoiceModelRunner {
		private readonly ActumHouseholdWrapper _household;
		private static object _lock = new object();

		public ActumChoiceModelRunner(IHousehold household) {
			_household = (ActumHouseholdWrapper) Global.Kernel.Get<HouseholdWrapperFactory>().HouseholdWrapperCreator.CreateWrapper(household);
		}
		...
		private void RunHouseholdDayModels() {
			...
			foreach (ActumHouseholdDayWrapper householdDay in _household.HouseholdDays) {
#if RELEASE
				try {
#endif
				lock (_lock) {
				...
				while (!householdDay.IsValid && (!Global.Configuration.IsInEstimationMode || !simulatedAnInvalidHouseholdDay)) { //don't retry household in estimation mode
					...
					RunHouseholdDayModelSuite((ActumHouseholdDayWrapper) householdDay);
		...
		private static void RunTourTripModels(ActumTourWrapper tour, ActumPersonDayWrapper personDay, ActumHouseholdDayWrapper householdDay, int firstDirection, int lastDirection) {
		...
		private static void RunSubtourModels(ActumTourWrapper tour, ActumPersonDayWrapper personDay, ActumHouseholdDayWrapper householdDay) {
			...
			foreach (ActumTourWrapper subtour in tour.Subtours) {
#if RELEASE
				try {
#endif
				lock (_lock) {
		...
		private static void RunSubtourTripModels(ActumTourWrapper subtour, ActumPersonDayWrapper personDay, ActumHouseholdDayWrapper householdDay, int firstDirection, int lastDirection) {
		...
		private void RunPersonModelSuite(IPersonWrapper person) {
			...
			if (!person.IsChildUnder5 && Global.Configuration.IncludeTransitPassOwnershipModel) {
				if (Global.Configuration.ShouldRunTransitPassOwnershipModel) {
					...
					(Global.ChoiceModelDictionary.Get("TransitPassOwnershipModel") as TransitPassOwnershipModel).Run(person);
				}
			}
//			else {
...
//			}
		...
		private static void RunHouseholdDayModelSuite(ActumHouseholdDayWrapper householdDay) {
			...
			if (Global.Configuration.ShouldRunActumPrimaryPriorityTimeModel) {
				...
				lock (_lock) {
					ChoiceModelFactory.TotalTimesActumPrimaryPriorityTimeModelRun++;
				}
				(Global.ChoiceModelDictionary.Get("ActumPrimaryPriorityTimeModel") as ActumPrimaryPriorityTimeModel).Run(householdDay);
				(Global.ChoiceModelDictionary.Get("ActumPrimaryPriorityTimeScheduleModel") as ActumPrimaryPriorityTimeScheduleModel).Run(householdDay);
			...
			if (Global.Configuration.ShouldRunHouseholdDayPatternTypeModel) {
				...
				(Global.ChoiceModelDictionary.Get("ActumHouseholdDayPatternTypeModel") as ActumHouseholdDayPatternTypeModel).Run(householdDay);
				...
				foreach (ActumPersonDayWrapper personDay in orderedPersonDays) {
					...
					if (i > 4 || (Global.Configuration.IsInEstimationMode && Global.Configuration.EstimationModel == "ActumPersonDayPatternTypeModel")) {
						...
						(Global.ChoiceModelDictionary.Get("ActumPersonDayPatternTypeModel") as ActumPersonDayPatternTypeModel).Run(personDay, householdDay);
					}
					//mbtrace
			...
			foreach (ActumPersonDayWrapper personDay in householdDay.PersonDays) {
				foreach (ActumTourWrapper tour in personDay.Tours) {
					if (tour.DestinationPurpose == Constants.Purpose.WORK
						...
						|| tour.DestinationPurpose == Constants.Purpose.BUSINESS) {
						lock (_lock) {
			...
			//					if (!Global.Configuration.IsInEstimationMode && personDay.Tours != null) {
			//						foreach (ActumTourWrapper tour in personDay.Tours.Where(tour => tour.Mode == Constants.Mode.PARK_AND_RIDE)) {
			//							tour.SetParkAndRideStay();
			//						}
			//					}
			//				}
		...
		private static void RunPersonDayMandatoryModelSuite(ActumPersonDayWrapper personDay, ActumHouseholdDayWrapper householdDay) {
			...
			if (Global.Configuration.ShouldRunWorkAtHomeModel) {
				...
				(Global.ChoiceModelDictionary.Get("ActumWorkAtHomeModel") as ActumWorkAtHomeModel).Run(personDay, householdDay);
				//mbtrace
			...
			if (Global.Configuration.ShouldRunMandatoryTourGenerationModel) {
				...
				if (personDay.PatternType == Constants.PatternType.MANDATORY && (personDay.Person.IsWorker || personDay.Person.IsStudent)) {
					...
					if (Global.Configuration.IsInEstimationMode) {
						...
						for (var i = 0; i <= totalMandatoryTours[0]; i++) {
							...
							ncallsfortour++;
							(Global.ChoiceModelDictionary.Get("ActumMandatoryTourGenerationModel") as ActumMandatoryTourGenerationModel).Run(personDay, householdDay, ncallsfortour, simulatedMandatoryTours, choice);
							if (choice > 0) {
						...
						for (var i = 0; i <= totalMandatoryTours[0]; i++) {
							...
							ncallsfortour++;
							var chosenAlternative = (Global.ChoiceModelDictionary.Get("ActumMandatoryTourGenerationModel") as ActumMandatoryTourGenerationModel).Run(personDay, householdDay, ncallsfortour, simulatedMandatoryTours);
					...
					(Global.ChoiceModelDictionary.Get("ActumMandatoryStopPresenceModel") as ActumMandatoryStopPresenceModel).Run(personDay, householdDay);
					//mbtrace
		...
		private static void RunJointHalfTourGenerationModelSuite(ActumHouseholdDayWrapper householdDay) {
			...
			for (var i = 0; i <= jHTSimulated; i++) {
				...
				nCallsForTour++;
				int genChoice = (Global.ChoiceModelDictionary.Get("ActumJointHalfTourGenerationModel") as ActumJointHalfTourGenerationModel).Run(householdDay, nCallsForTour, jHTAvailable, type, subType);
				...
				if (genChoice > 0 && genChoice <= 3) {
					...
					participants = (Global.ChoiceModelDictionary.Get("ActumFullJointHalfTourParticipationModel") as ActumFullJointHalfTourParticipationModel).Run(householdDay, jHTSimulated, genChoice, hTAvailable, fHTAvailable, jHTParticipation);
							...
							foreach (ActumPersonDayWrapper personDay in orderedPersonDays) {
								...
								if (count3 <= maxNumberParticipants) {
									if (participants[count3] == 1 &&
										...
										ActumTourWrapper tour = (ActumTourWrapper) personDay.GetEscortTour(Constants.AddressType.HOME, personDay.Household.ResidenceParcelId, personDay.Household.ResidenceZoneKey);
									...
									//update mandatory tour of mandatory participants with FullJointHalfTour index
										...
										if (personDay.Person.IsFullOrPartTimeWorker) {
											ActumTourWrapper tour = (ActumTourWrapper) personDay.Tours.FirstOrDefault(x => x.IsWorkPurpose == true && x.DestinationParcelId == personDay.Person.UsualWorkParcelId);
											...
											if (!(tour == null)) {
												...
												tour = (ActumTourWrapper) personDay.Tours.FirstOrDefault(x => x.IsSchoolPurpose == true);
											...
											ActumTourWrapper tour = (ActumTourWrapper) personDay.Tours.FirstOrDefault(x => x.IsSchoolPurpose == true);
											...
											if (!(tour == null)) {
												...
												tour = (ActumTourWrapper) personDay.Tours.FirstOrDefault(x => x.IsWorkPurpose == true && x.DestinationParcelId == personDay.Person.UsualWorkParcelId);
					...
					participants = (Global.ChoiceModelDictionary.Get("ActumPartialJointHalfTourParticipationModel") as ActumPartialJointHalfTourParticipationModel).Run(householdDay, jHTSimulated, genChoice - 3, hTAvailable, pHTAvailable, jHTParticipation);
					int chauffeurSequence = (Global.ChoiceModelDictionary.Get("ActumPartialJointHalfTourChauffeurModel") as ActumPartialJointHalfTourChauffeurModel).Run(householdDay, jHTSimulated, genChoice - 3, participants, jHTChauffeurSequence);
					if (!Global.Configuration.IsInEstimationMode) {
						...
						ICondensedParcel[] destinationParcel = new ICondensedParcel[6];
		...
		private static void RunJointTourGenerationModelSuite(ActumHouseholdDayWrapper householdDay) {
			...
			for (var i = 0; i < 8; i++) {
				...
				nCallsForTour++;
				purpose[nCallsForTour] = (Global.ChoiceModelDictionary.Get("ActumJointTourGenerationModel") as ActumJointTourGenerationModel).Run(householdDay, nCallsForTour, purpose[nCallsForTour]);
				if (purpose[nCallsForTour] > Constants.Purpose.NONE_OR_HOME) {
					...
					int[] participants = (Global.ChoiceModelDictionary.Get("ActumJointTourParticipationModel") as ActumJointTourParticipationModel).Run(householdDay, nCallsForTour, purpose, jTParticipation);
						...
						foreach (ActumPersonDayWrapper personDay in orderedPersonDays) {
							...
							if (count <= maxNumberParticipants) {
								if (participants[count] == 1) {
									...
									ActumTourWrapper tour = (ActumTourWrapper) personDay.GetNewTour(Constants.AddressType.HOME, personDay.Household.ResidenceParcelId, personDay.Household.ResidenceZoneKey, purpose[nCallsForTour]);
				...
				orderedPersonDays = householdDay.PersonDays.OrderBy(p => p.JointTourParticipationPriority).ToList().Cast<ActumPersonDayWrapper>();
		...
		private static void RunPersonDayModelSuite(ActumPersonDayWrapper personDay, ActumHouseholdDayWrapper householdDay) {
			...
			if (Global.Configuration.ShouldRunHouseholdPersonDayPatternModel) {
				...
				(Global.ChoiceModelDictionary.Get("ActumPersonDayPatternModel") as ActumPersonDayPatternModel).Run(personDay, householdDay);
			}
			...
			for (var count = 1; count <= 10; count++) {
				...
				tourPurpose[count] = (Global.ChoiceModelDictionary.Get("ActumPersonTourGenerationModel") as ActumPersonTourGenerationModel).Run(personDay, householdDay, maxPurpose, tourPurpose[count]);
				...
				if (!Global.Configuration.IsInEstimationMode) {
					ActumTourWrapper tour = (ActumTourWrapper) personDay.GetNewTour(Constants.AddressType.HOME, personDay.Household.ResidenceParcelId, personDay.Household.ResidenceZoneKey, tourPurpose[count]);
					//mbtrace
		...
		private static void RunPartialJointHalfTourModelSuite(ActumHouseholdDayWrapper householdDay, IPartialHalfTourWrapper partialJointHalfTour) {
			...
			ActumPersonDayWrapper[] participantDay = new ActumPersonDayWrapper[9];
			ActumTourWrapper[] tour = new ActumTourWrapper[9];
			IHalfTour[] halfTour = new IHalfTour[9];

			//get partialJointHalfTour.TourSequence1 thru 8 tours into tour array
			participantDay[1] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == partialJointHalfTour.PersonSequence1);
			participantDay[2] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == partialJointHalfTour.PersonSequence2);
			participantDay[3] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == partialJointHalfTour.PersonSequence3);
			participantDay[4] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == partialJointHalfTour.PersonSequence4);
			participantDay[5] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == partialJointHalfTour.PersonSequence5);
			participantDay[6] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == partialJointHalfTour.PersonSequence6);
			participantDay[7] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == partialJointHalfTour.PersonSequence7);
			participantDay[8] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == partialJointHalfTour.PersonSequence8);
			//get partialJointHalfTour.PersonSequence1 thru 8 personDays into particpantDay array
			tour[1] = participantDay[1] == null ? null : (ActumTourWrapper) participantDay[1].Tours.FirstOrDefault(t => t.Sequence == partialJointHalfTour.TourSequence1);
			tour[2] = participantDay[2] == null ? null : (ActumTourWrapper) participantDay[2].Tours.FirstOrDefault(t => t.Sequence == partialJointHalfTour.TourSequence2);
			tour[3] = participantDay[3] == null ? null : (ActumTourWrapper) participantDay[3].Tours.FirstOrDefault(t => t.Sequence == partialJointHalfTour.TourSequence3);
			tour[4] = participantDay[4] == null ? null : (ActumTourWrapper) participantDay[4].Tours.FirstOrDefault(t => t.Sequence == partialJointHalfTour.TourSequence4);
			tour[5] = participantDay[5] == null ? null : (ActumTourWrapper) participantDay[5].Tours.FirstOrDefault(t => t.Sequence == partialJointHalfTour.TourSequence5);
			tour[6] = participantDay[6] == null ? null : (ActumTourWrapper) participantDay[6].Tours.FirstOrDefault(t => t.Sequence == partialJointHalfTour.TourSequence6);
			tour[7] = participantDay[7] == null ? null : (ActumTourWrapper) participantDay[7].Tours.FirstOrDefault(t => t.Sequence == partialJointHalfTour.TourSequence7);
			tour[8] = participantDay[8] == null ? null : (ActumTourWrapper) participantDay[8].Tours.FirstOrDefault(t => t.Sequence == partialJointHalfTour.TourSequence8);
			...
			ICondensedParcel[] destination = new CondensedParcel[9];
			...
			for (var i = 2; i <= partialJointHalfTour.Participants; i++) {
				...
				if (!tour[i].DestinationModeAndTimeHaveBeenSimulated) {
					if (!(Global.Configuration.IsInEstimationMode)) {
						tour[i].Mode = Constants.Mode.HOVPASSENGER;
						// TODO: need to model mode for Actum
					}
					if (i == 2) {
						...
						if (tour[i].HalfTourFromOrigin == null) {
							...
							if (!(Global.Configuration.IsInEstimationMode)) {
								...
								var trip = (ActumTripWrapper) hT.Trips[0];
			...
			for (var i = 1; i <= partialJointHalfTour.Participants; i++) {  // this loops on all participants
				for (var j = halfTourTrips[1] - halfTourTrips[i] + 1; j <= halfTourTrips[1]; j++) {  // this loops on the trips for the participant
					var trip = (ActumTripWrapper) halfTour[i].Trips[j + halfTourTrips[i] - halfTourTrips[1] - 1];
					...
					if (i == 1) {
						if (travelers[j] == 1) {
							tripMode = Constants.Mode.SOV;
						}
						else {
							tripMode = Constants.Mode.HOVDRIVER;
						}
					}
					else {
						tripMode = Constants.Mode.HOVPASSENGER;
					}
					if (direction == 1) {
		...
		private static void RunFullJointHalfTourModelSuite(ActumHouseholdDayWrapper householdDay, IFullHalfTourWrapper fullJointHalfTour) {
			...
			participantDay[1] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == fullJointHalfTour.PersonSequence1);
			participantDay[2] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == fullJointHalfTour.PersonSequence2);
			participantDay[3] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == fullJointHalfTour.PersonSequence3);
			participantDay[4] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == fullJointHalfTour.PersonSequence4);
			participantDay[5] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == fullJointHalfTour.PersonSequence5);
			participantDay[6] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == fullJointHalfTour.PersonSequence6);
			participantDay[7] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == fullJointHalfTour.PersonSequence7);
			participantDay[8] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == fullJointHalfTour.PersonSequence8);
			//get fullJointHalfTour.PersonSequence1 thru 8 personDays into particpantDay array
			tour[1] = participantDay[1] == null ? null : (ActumTourWrapper) participantDay[1].Tours.FirstOrDefault(t => t.Sequence == fullJointHalfTour.TourSequence1);
			tour[2] = participantDay[2] == null ? null : (ActumTourWrapper) participantDay[2].Tours.FirstOrDefault(t => t.Sequence == fullJointHalfTour.TourSequence2);
			tour[3] = participantDay[3] == null ? null : (ActumTourWrapper) participantDay[3].Tours.FirstOrDefault(t => t.Sequence == fullJointHalfTour.TourSequence3);
			tour[4] = participantDay[4] == null ? null : (ActumTourWrapper) participantDay[4].Tours.FirstOrDefault(t => t.Sequence == fullJointHalfTour.TourSequence4);
			tour[5] = participantDay[5] == null ? null : (ActumTourWrapper) participantDay[5].Tours.FirstOrDefault(t => t.Sequence == fullJointHalfTour.TourSequence5);
			tour[6] = participantDay[6] == null ? null : (ActumTourWrapper) participantDay[6].Tours.FirstOrDefault(t => t.Sequence == fullJointHalfTour.TourSequence6);
			tour[7] = participantDay[7] == null ? null : (ActumTourWrapper) participantDay[7].Tours.FirstOrDefault(t => t.Sequence == fullJointHalfTour.TourSequence7);
			tour[8] = participantDay[8] == null ? null : (ActumTourWrapper) participantDay[8].Tours.FirstOrDefault(t => t.Sequence == fullJointHalfTour.TourSequence8);
			...
			ActumTourWrapper sourceTour = null;
			for (var i = 1; i <= 8; i++) {
				if (!(participantDay[i] == null) && oldestNonEscortPersonID == participantDay[i].Person.Id) {
		...
		private static void RunMandatoryTourModelSuite(ActumTourWrapper tour, ActumPersonDayWrapper personDay, ActumHouseholdDayWrapper householdDay) {
		...
		private static void RunJointTourModelSuite(ActumHouseholdDayWrapper householdDay, IJointTourWrapper jointTour) {
			//mbtrace
			...
			participantDay[1] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == jointTour.PersonSequence1);
			participantDay[2] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == jointTour.PersonSequence2);
			participantDay[3] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == jointTour.PersonSequence3);
			participantDay[4] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == jointTour.PersonSequence4);
			participantDay[5] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == jointTour.PersonSequence5);
			participantDay[6] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == jointTour.PersonSequence6);
			participantDay[7] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == jointTour.PersonSequence7);
			participantDay[8] = (ActumPersonDayWrapper) householdDay.PersonDays.FirstOrDefault(pDay => pDay.Person.Sequence == jointTour.PersonSequence8);
			//get jointTour.PersonSequence1 thru 8 personDays into particpantDay array
			tour[1] = participantDay[1] == null ? null : (ActumTourWrapper) participantDay[1].Tours.FirstOrDefault(t => t.Sequence == jointTour.TourSequence1);
			tour[2] = participantDay[2] == null ? null : (ActumTourWrapper) participantDay[2].Tours.FirstOrDefault(t => t.Sequence == jointTour.TourSequence2);
			tour[3] = participantDay[3] == null ? null : (ActumTourWrapper) participantDay[3].Tours.FirstOrDefault(t => t.Sequence == jointTour.TourSequence3);
			tour[4] = participantDay[4] == null ? null : (ActumTourWrapper) participantDay[4].Tours.FirstOrDefault(t => t.Sequence == jointTour.TourSequence4);
			tour[5] = participantDay[5] == null ? null : (ActumTourWrapper) participantDay[5].Tours.FirstOrDefault(t => t.Sequence == jointTour.TourSequence5);
			tour[6] = participantDay[6] == null ? null : (ActumTourWrapper) participantDay[6].Tours.FirstOrDefault(t => t.Sequence == jointTour.TourSequence6);
			tour[7] = participantDay[7] == null ? null : (ActumTourWrapper) participantDay[7].Tours.FirstOrDefault(t => t.Sequence == jointTour.TourSequence7);
			tour[8] = participantDay[8] == null ? null : (ActumTourWrapper) participantDay[8].Tours.FirstOrDefault(t => t.Sequence == jointTour.TourSequence8);
		...
		private static void RunNonMandatoryTourModelSuite(ActumTourWrapper tour, ActumPersonDayWrapper personDay, ActumHouseholdDayWrapper householdDay) {
#if RELEASE
				try {
#endif
			//mbtrace
		...
		private static void UpdateTimeWindowForTourDestinationTimes(ActumTourWrapper tour) {
		...
		private static void SetTourDestination(ActumTourWrapper tour, ActumHouseholdDayWrapper householdDay, ICondensedParcel constrainedParcel = null) {
			//mbtrace
			...
			switch (tour.DestinationPurpose) {
				case Constants.Purpose.BUSINESS:
					if (Global.Configuration.ShouldRunTourDestinationModel) {
						...
						lock (_lock) {
							ChoiceModelFactory.TotalTimesTourDestinationModelRun++;
						}
						(Global.ChoiceModelDictionary.Get("ActumTourDestinationModel") as ActumTourDestinationModel).Run(tour, householdDay, Global.Configuration.TourDestinationModelSampleSize, constrainedParcel);
					}
		...
		private static void GenerateSubtours(ActumTourWrapper tour, ActumHouseholdDayWrapper householdDay) {
			//mbtrace
			...
			if (Global.Configuration.IsInEstimationMode) {
				...
				foreach (var subtour in tour.Subtours) {
					...
					nCallsForTour++;
					(Global.ChoiceModelDictionary.Get("ActumWorkBasedSubtourGenerationModel") as ActumWorkBasedSubtourGenerationModel).Run(tour, householdDay, nCallsForTour, subtour.DestinationPurpose);
				}
				nCallsForTour++;
				lock (_lock) {
					ChoiceModelFactory.TotalTimesWorkBasedSubtourGenerationModelRun++;
				}
				(Global.ChoiceModelDictionary.Get("ActumWorkBasedSubtourGenerationModel") as ActumWorkBasedSubtourGenerationModel).Run(tour, householdDay, nCallsForTour);
			}
			else {
				...
				while (tour.Subtours.Count < 4 && tour.PersonDay.TotalCreatedTours < 10) {
					...
					nCallsForTour++;
					var destinationPurposeForSubtour = (Global.ChoiceModelDictionary.Get("ActumWorkBasedSubtourGenerationModel") as ActumWorkBasedSubtourGenerationModel).Run(tour, householdDay, nCallsForTour);
		...
		private static void SetTourModeAndTime(ActumHouseholdDayWrapper householdDay, ActumTourWrapper tour,
			...
			if (Global.Configuration.ShouldRunTourModeTimeModel) {
				...
				(Global.ChoiceModelDictionary.Get("ActumTourModeTimeModel") as ActumTourModeTimeModel).Run(householdDay, tour, constrainedMode, constrainedArrivalTime, constrainedDepartureTime);
		...
		private static void RunSubtourModelSuite(ActumTourWrapper subtour, ActumHouseholdDayWrapper householdDay) {
			//mbtrace
		...
		private static void SetSubtourDestination(ActumTourWrapper subtour, ActumHouseholdDayWrapper householdDay) {
			//mbtrace
			...
			switch (subtour.DestinationPurpose) {
				case Constants.Purpose.BUSINESS:
					if (Global.Configuration.ShouldRunTourDestinationModel) {
						// sets the destination for the business tour
						lock (_lock) {
							ChoiceModelFactory.TotalTimesBusinessSubtourDestinationModelRun++;
						}
						(Global.ChoiceModelDictionary.Get("ActumTourDestinationModel") as ActumTourDestinationModel).Run(subtour, householdDay, Global.Configuration.TourDestinationModelSampleSize);
					}
		...
		private static void SetSubtourModeAndTime(ActumHouseholdDayWrapper householdDay, ActumTourWrapper subtour) {
			//mbtrace
			...
			if (Global.Configuration.ShouldRunTourModeTimeModel) {
				...
				if (subtour.JointTourSequence > 0) {
					foreach (ActumPersonDayWrapper pDay in householdDay.PersonDays) {
						var tInJoint = (ActumTourWrapper) pDay.Tours.Find(t => t.JointTourSequence == subtour.JointTourSequence);
						if (!(tInJoint == null)) {
							// set jointTour time window
					...
					foreach (ActumPersonDayWrapper pDay in householdDay.PersonDays) {
						var tInJoint = (ActumTourWrapper) pDay.Tours.Find(t => t.FullHalfTour1Sequence == subtour.FullHalfTour1Sequence);
						if (!(tInJoint == null)) {
							// set jointTour time window
							timeWindow.IncorporateAnotherTimeWindow(tInJoint.PersonDay.TimeWindow);
						}
					...
					foreach (ActumPersonDayWrapper pDay in householdDay.PersonDays) {
						var tInJoint = (ActumTourWrapper) pDay.Tours.Find(t => t.FullHalfTour2Sequence == subtour.FullHalfTour2Sequence);
						if (!(tInJoint == null)) {
							// set jointTour time window
							timeWindow.IncorporateAnotherTimeWindow(tInJoint.PersonDay.TimeWindow);
						}
				...
				(Global.ChoiceModelDictionary.Get("ActumTourModeTimeModel") as ActumTourModeTimeModel).Run(householdDay, subtour, 0, 0, 0);
		...
		private static void RunPartialHalfTourTripModelSuite(ActumTourWrapper tour, IHalfTour halfTour, ActumTripWrapper trip, int stopPurpose, ICondensedParcel stopLocation, int mode, int departureTime) {
			//mbtrace
			...
			var nextTrip = (ActumTripWrapper) GeneratePartialHalfTourIntermediateStop(halfTour, trip, stopPurpose, stopLocation, mode, departureTime);
		...
		private static TripWrapper GeneratePartialHalfTourIntermediateStop(IHalfTour halfTour, ActumTripWrapper trip, int stopPurpose, ICondensedParcel stopLocation, int mode, int departureTime) {
			...
			ActumTripWrapper nextTrip = null;
			...
			if (Global.Configuration.IsInEstimationMode) {
				...
				if (stopPurpose != Constants.Purpose.NONE_OR_HOME) {
					...
					nextTrip = (ActumTripWrapper) halfTour.CreateNextTrip(trip, stopPurpose, destinationPurposeForNextTrip);
		...
		private static void SetPartialHalfTourIntermediateStopDestination(ActumTripWrapper trip, ActumTripWrapper nextTrip, int stopPurpose, ICondensedParcel stopLocation, int mode, int departureTime) {
			//mbtrace
		...
		private static void SetPartialHalfTourTripModeAndTime(ActumTourWrapper tour, ActumTripWrapper trip, int stopPurpose, ICondensedParcel stopLocation, int mode, int departureTime) {
			//mbtrace
		...
		private static void ProcessHalfTours(ActumTourWrapper tour, ActumPersonDayWrapper personDay, ActumHouseholdDayWrapper householdDay, int firstDirection, int lastDirection) {
			//mbtrace
			...
			for (var direction = firstDirection; direction <= lastDirection; direction++) {
				...
				for (var i = 0; i < halfTour.Trips.Count; i++) {
					var trip = (ActumTripWrapper) halfTour.Trips[i];
		...
		private static void RunTripModelSuite(ActumHouseholdDayWrapper householdDay, ActumPersonDayWrapper personDay, ActumTourWrapper tour, IHalfTour halfTour, ActumTripWrapper trip) {
			//mbtrace
			...
			var nextTrip = (ActumTripWrapper) GenerateIntermediateStop(halfTour, trip, householdDay);
		...
		private static ActumTripWrapper GenerateIntermediateStop(IHalfTour halfTour, ActumTripWrapper trip, ActumHouseholdDayWrapper householdDay) {
			//mbtrace
			...
			if (Global.Configuration.IsInEstimationMode) {
				...
				trip.IsToTourOrigin = trip.Sequence == trip.HalfTour.Trips.Count(); // last trip in half tour 
				var intermediateStopPurpose = trip.IsToTourOrigin ? Constants.Purpose.NONE_OR_HOME : trip.DestinationPurpose;
				nextTrip = (ActumTripWrapper) trip.NextTrip;
				...
				if (trip.PersonDay.TotalStops > 0) {
					...
					(Global.ChoiceModelDictionary.Get("ActumIntermediateStopGenerationModel") as ActumIntermediateStopGenerationModel).Run(trip, householdDay, intermediateStopPurpose);
				}
			}
			else {
				// -- in application mode --
				...
				if (trip.Sequence == 1 && trip.Tour.Mode == Constants.Mode.PARK_AND_RIDE) {
					...
					intermediateStopPurpose = (Global.ChoiceModelDictionary.Get("ActumIntermediateStopGenerationModel") as ActumIntermediateStopGenerationModel).Run(trip, householdDay);
				}
				...
				if (intermediateStopPurpose != Constants.Purpose.NONE_OR_HOME) {
					...
					// creates the next trip in the half-tour 
					// the next trip's destination is set to the current trip's destination
					nextTrip = (ActumTripWrapper) halfTour.CreateNextTrip(trip, intermediateStopPurpose, destinationPurposeForNextTrip);
		...
		private static void SetIntermediateStopDestination(ActumTripWrapper trip, ActumTripWrapper nextTrip, ActumHouseholdDayWrapper householdDay) {
			//mbtrace
			...
			if (trip.DestinationPurpose == Constants.Purpose.CHANGE_MODE) {
				...
				(Global.ChoiceModelDictionary.Get("ActumIntermediateStopLocationModel") as ActumIntermediateStopLocationModel).Run(trip, householdDay, Global.Configuration.IntermediateStopLocationModelSampleSize);
			}
			if (Global.Configuration.IsInEstimationMode) {
				return;
			}
		...
		private static void SetTripModeAndTime(ActumHouseholdDayWrapper householdDay, ActumTourWrapper tour, ActumTripWrapper trip, bool forTourTimesOnly = false) {
			//mbtrace
			...
			if (Global.Configuration.ShouldRunTripModeModel) {
				...
				if (trip.DestinationPurpose == Constants.Purpose.CHANGE_MODE) {
					...
					(Global.ChoiceModelDictionary.Get("ActumTripModeModel") as ActumTripModeModel).Run(householdDay, trip);
			...
			if (trip.OriginPurpose == Constants.Purpose.CHANGE_MODE) {
				...
				if (Global.Configuration.ShouldRunTripTimeModel) {
					...
					(Global.ChoiceModelDictionary.Get("ActumTripTimeModel") as ActumTripTimeModel).Run(householdDay, trip);
					...
					if (trip.PersonDay.IsValid) {
						if (!forTourTimesOnly) {
							if (trip.Mode == Constants.Mode.WALK || trip.Mode == Constants.Mode.BIKE) {
								testbreak = true;
							}
							trip.HUpdateTripValues();
						}
		...
		private static void CloneHalfTours(ActumTourWrapper sourceTour, ActumTourWrapper tour, int firstDirection, int lastDirection) {
			//mbtrace
			...
			for (var direction = firstDirection; direction <= lastDirection; direction++) {
				...
				for (var i = 0; i < sourceHalfTour.Trips.Count; i++) {
					var trip = (ActumTripWrapper) halfTour.Trips[i];
					var sourceTrip = (ActumTripWrapper) sourceHalfTour.Trips[i];
		...
		private static void RunTripCloneSuite(ActumTourWrapper sourceTour, IHalfTour sourceHalfTour, ActumTripWrapper sourceTrip, ActumTourWrapper tour, IHalfTour halfTour, ActumTripWrapper trip) {
			//mbtrace
			...
			var nextTrip = (ActumTripWrapper) GenerateIntermediateStopClone(sourceHalfTour, sourceTrip, halfTour, trip);
		...
		private static TripWrapper GenerateIntermediateStopClone(IHalfTour sourceHalfTour, ActumTripWrapper sourceTrip, IHalfTour halfTour, ActumTripWrapper trip) {
			//mbtrace
		...
		private static void CloneIntermediateStopDestination(ActumTripWrapper sourceTrip, ActumTripWrapper trip, ActumTripWrapper nextTrip) {
			//mbtrace
		...
		private static void CloneTripModeAndTime(ActumTourWrapper sourceTour, ActumTripWrapper sourceTrip, ActumTourWrapper tour, ActumTripWrapper trip) {
			//mbtrace
		...
		private void UpdateHousehold() {
			...
			foreach (ActumPersonWrapper person in _household.Persons) {
				person.UpdatePersonValues();
			}

			if (Global.Configuration.ShouldRunTourModels) {

				foreach (ActumTourWrapper tour in _household.HouseholdDays.SelectMany(householdDay => householdDay.PersonDays.Where(personDay => personDay.Tours != null)).SelectMany(personDay => personDay.Tours)) {
				...
				foreach (ActumPersonDayWrapper personDay in householdDay.PersonDays) {
					...
					foreach (ActumTourWrapper tour in personDay.Tours) {
						...
						if (tour.HalfTourFromOrigin != null && tour.HalfTourFromDestination != null) {
							foreach (ActumTripWrapper trip in tour.HalfTourFromOrigin.Trips.Invert()) {
							...
							foreach (ActumTripWrapper trip in tour.HalfTourFromDestination.Trips) {
								trip.SetTourSequence(tour.Sequence);
								trip.SetTripValueOfTime();
								trip.Export();
						...
						foreach (ActumTourWrapper subtour in tour.Subtours) {
							...
							foreach (ActumTripWrapper trip in subtour.HalfTourFromOrigin.Trips.Invert()) {
							...
							foreach (ActumTripWrapper trip in subtour.HalfTourFromDestination.Trips) {
								trip.SetTourSequence(subtour.Sequence);
								trip.Export();
