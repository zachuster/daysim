...
namespace Daysim.ChoiceModels.H {
	public class HMandatoryStopPresenceModel : ChoiceModel {
		...
		private void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, IPersonDayWrapper personDay, IHouseholdDayWrapper householdDay, int choice = Constants.DEFAULT_VALUE) {
            ...
            if (personDay.Person.UsualSchoolParcelId != 0 && personDay.Person.UsualSchoolParcelId != -1 && personDay.Person.UsualSchoolParcelId != Constants.OUT_OF_REGION_PARCEL_ID)
            {
                ...
                var schoolNestedAlternative = (Global.ChoiceModelDictionary.Get("HSchoolTourModeTimeModel") as HSchoolTourModeTimeModel).RunNested(personDay.Person, personDay.Person.Household.ResidenceParcel, personDay.Person.UsualSchoolParcel, Constants.Time.EIGHT_AM, Constants.Time.TWO_PM, personDay.Person.Household.HouseholdTotals.DrivingAgeMembers);
                schoolPclStudents= Math.Log(1 + (personDay.Person.UsualSchoolParcel.StudentsK12)) / 10;
            }
