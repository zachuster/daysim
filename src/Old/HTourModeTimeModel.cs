...
namespace Daysim.ChoiceModels {
	public class HTourModeTimeModel : ChoiceModel {
		...
		{
			Initialize(CHOICE_MODEL_NAME, Global.Configuration.TourModeTimeModelCoefficients, HTourModeTime.TOTAL_TOUR_MODE_TIMES, TOTAL_NESTED_ALTERNATIVES, TOTAL_LEVELS, MAX_PARAMETER);
		}
		...
		private ChoiceProbabilityCalculator.Alternative RunNested(ITourWrapper tour, ICondensedParcel destinationParcel, int householdCars, double transitDiscountFraction) {
			...
			RunModel(choiceProbabilityCalculator, householdDay, tour, destinationParcel, householdCars, 0, 0, 0 );
		...
		private void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, IHouseholdDayWrapper householdDay, ITourWrapper tour,
					ICondensedParcel destinationParcel, int householdCars,
			...
			for (var mode = Constants.Mode.WALK; mode <= Constants.Mode.SCHOOL_BUS; mode++) {
				...
				modeComponent.AddUtilityTerm(298, mode>=Constants.Mode.SOV && mode<=Constants.Mode.HOV3 && parentTourMode==Constants.Mode.SOV ? 1:0 );
				modeComponent.AddUtilityTerm(299, mode>=Constants.Mode.SOV && mode<=Constants.Mode.HOV3 && parentTourMode>=Constants.Mode.HOV2 && parentTourMode<=Constants.Mode.HOV3 ? 1:0 );
			}
			...
			//loop on all alternatives, using modeTimes objects
			{
				foreach (var modeTimes in HTourModeTime.ModeTimes[ParallelUtility.GetBatchFromThreadId()])
				{
					...
					var destinationParkingCost = (!Global.Configuration.IsInEstimationMode
					                              ...
					                              person.PayToParkAtWorkplaceFlag == 0)
						                             ? 0.0
						                             : destinationParcel.ParkingCostBuffer1(parkingDuration);
					var parkingCostFraction = (mode == Constants.Mode.SOV)
						                          ? 1.0
					...
					var minimumTimeNeeded = modeTimes.TravelTimeToDestination + modeTimes.TravelTimeFromDestination +
					                        Constants.Time.MINIMUM_ACTIVITY_DURATION;
					...
					alternative.AddUtilityTerm(5,
					                           (maleFlag == 0 && mode == Constants.Mode.WALK &&
