...
namespace Daysim.ChoiceModels.H {
	public class HFullJointHalfTourParticipationModel : ChoiceModel {
		...
		private void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, HouseholdDayWrapper householdDay, int jHTSimulated, int genChoice, bool[,] jHTAvailable, bool[] fHTAvailable, int[][] altParticipants, int choice = Constants.DEFAULT_VALUE) {
			...
			for (int alt = 0; alt < 32; alt++) {
				...
				var nestedAlternative = (Global.ChoiceModelDictionary.Get("HWorkTourModeTimeModel") as HWorkTourModeTimeModel).RunNested(pPersonDay, pPersonDay.Household.ResidenceParcel, sameUsualLocation, destinationArrivalTime, destinationDepartureTime, pPersonDay.Household.VehiclesAvailable);
