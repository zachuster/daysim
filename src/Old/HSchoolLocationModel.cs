...
{
    public class HSchoolLocationModel : ChoiceModel
    {
        ...
        private void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, IPersonWrapper person, int sampleSize, ICondensedParcel choice = null, bool choseHome = false)
        {
        ...
        private sealed class SchoolLocationUtilities : ISamplingUtilities
        {
            ...
            public void SetUtilities(ISampleItem sampleItem, int sampleFrequency)
            {
                ...
                var nestedAlternative = (Global.ChoiceModelDictionary.Get("HSchoolTourModeTimeModel") as HSchoolTourModeTimeModel).RunNested(_person, _person.Household.ResidenceParcel, destinationParcel, _destinationArrivalTime, _destinationDepartureTime, _person.Household.HouseholdTotals.DrivingAgeMembers);
            ...
            foreach (PersonWrapper otherStu in person.Household.Persons)
            {
                if (otherStu.UsualSchoolParcelId > 0)
                {
                    ...
                    if (random.Uniform01() < thisPr)
                    {
                        ...
                        var skimValue = ImpedanceRoster.GetValue("ivtime", Constants.Mode.SOV, Constants.PathType.FULL_NETWORK, Constants.ValueOfTime.DEFAULT_VOT, 1, person.Household.ResidenceParcel, person.UsualSchoolParcel);
